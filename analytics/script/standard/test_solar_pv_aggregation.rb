...
script = {
  logger1: { name: TestDirectory.instance.log_directory + '/pv %{time}.log',
  ...
  reports: {
    ...
    control: {
      ...
      compare_results: [
        { comparison_directory: ENV.fetch('ANALYTICSTESTRESULTDIR',
                                          nil) + '\SolarPVAggregation\Base' },
        { output_directory: ENV.fetch('ANALYTICSTESTRESULTDIR',
                                      nil) + '\SolarPVAggregation\New' },
        :summary,
        :quick_comparison
      ]
    }
