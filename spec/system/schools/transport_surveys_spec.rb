      ...
      context 'with a zero carbon result' do
        before do
          ...
          create(:transport_survey_response, transport_survey: transport_survey, transport_type: transport_type,
                                             passengers: 1, journey_minutes: 5)
          visit school_transport_survey_path(school, transport_survey)
        end

        it {
          expect(page).to have_text('1 pupil or staff member included in this survey generated 0.0kg carbon by travelling to school')
        }
        ...
        it 'displays a table of transport type results' do
          within('table#responses_per_category') do
            rows = [['Walking and cycling', 1, '100%'], ['Car', 0, '0%'], ['Public transport', 0, '0%'],
                    ['Park and stride', 0, '0%'], ['Other', 0, '0%']]
            rows.each do |row|
        ...
        it {
          expect(page).to have_text("Persuading people who travel only 5 minutes by car to switch to coming to school by foot or bike is a great first step to reducing the school's carbon footprint from travel.")
        }

        it 'displays a table of times travelled by car' do
          within 'table#time_travelled_by_car' do
            rows = [['5 minutes', 0], ['10 minutes', 0], ['15 minutes', 0], ['20 minutes', 0],
                    ['30 or more minutes', 0]]
            rows.each do |row|
      ...
      context 'with a carbon positive result' do
        before do
          categories.each do |cat|
            ...
            create(:transport_survey_response, transport_survey: transport_survey, transport_type: transport_type,
                                               passengers: 1, journey_minutes: journey_minutes)
          end
          visit school_transport_survey_path(school, transport_survey)
        end
        ...
        context 'with one result' do
          ...
          it {
            expect(page).to have_text('1 pupil or staff member included in this survey generated 0.46kg carbon by travelling to school')
          }
          ...
          it 'displays a table of transport type results' do
            within('table#responses_per_category') do
              rows = [['Walking and cycling', 0, '0%'], ['Car', 1, '100%'], ['Public transport', 0, '0%'],
                      ['Park and stride', 0, '0%'], ['Other', 0, '0%']]
              rows.each do |row|
                expect(page).to have_selector(:table_row, cat_cols.zip(row).to_h)
              end
          ...
          it 'displays a table of times travelled by car' do
            within 'table#time_travelled_by_car' do
              rows = [['5 minutes', 1], ['10 minutes', 0], ['15 minutes', 0], ['20 minutes', 0],
                      ['30 or more minutes', 0]]
              rows.each do |row|
                expect(page).to have_selector(:table_row, car_cols.zip(row).to_h)
              end
        ...
        context 'with one result per category' do
          ...
          it {
            expect(page).to have_text('5 pupils and staff included in this survey generated 2.29kg carbon by travelling to school')
          }
          ...
          it 'displays a table of transport type results' do
            rows = [['Walking and cycling', 1, '20%'], ['Car', 1, '20%'], ['Public transport', 1, '20%'],
                    ['Park and stride', 1, '20%'], ['Other', 1, '20%']]
            within 'table#responses_per_category' do
              rows.each do |row|
                expect(page).to have_selector(:table_row, cat_cols.zip(row).to_h)
              end
  ...
  describe 'Abilities' do
    ...
    managing_user_types.each do |user_type|
      describe "as a #{user_type} user who can delete surveys and manage & delete responses" do
        let!(:user) { create(user_type) }
        let!(:transport_survey) { create(:transport_survey, school: school) }
        let!(:transport_survey_response) do
          create(:transport_survey_response, transport_survey: transport_survey, transport_type: transport_type)
        end
          ...
          context 'when managing responses' do
            ...
            context 'when deleting response' do
              ...
              it 'removes response' do
                expect(page).to have_no_text(nice_date_times(transport_survey_response.surveyed_at,
                                                             localtime: true))
              end
            end
