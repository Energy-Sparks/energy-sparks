...
describe 'Pupil dashboard' do
  ...
  let!(:school)             do
    create(:school, :with_feed_areas, calendar: calendar, name: school_name, school_group: school_group)
  end
  ...
  shared_examples 'a data enabled pupil dashboard' do
    ...
    it 'hides old equivalences' do
      ...
      expect(page).to have_no_text('Your school spent £2.00 on electricity last year!')
    end
  ...
  shared_examples 'a non-data enabled pupil dashboard' do
    ...
    it 'shows default equivalences' do
      ...
      expect(page).to have_text('How will your school compare?')
    end
  end
  ...
  context 'when logged in as pupil' do
    ...
    it 'has navigation to adult dashboard' do
      expect(page).to have_text(school.name.to_s)
