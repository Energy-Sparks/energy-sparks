...
shared_examples_for 'a displayed procurement route' do
  it 'displays procurement route fields' do
    text_attributes.each_key do |text_field|
      expect(page).to have_text(procurement_route[text_field])
    end
  end
end
...
RSpec.describe 'Procurement route admin', :include_application_helper, :school_groups, type: :system do
  let(:setup_data)             {}
  let!(:user)                  {}
  ...
  describe 'Unauthorized access' do
    ...
    context 'as a non-admin user' do
      ...
      it { expect(page).to have_text('You are not authorized to view that page.') }
    end
  end
    ...
    describe 'Viewing index page' do
      ...
      context 'when there is a procurement route' do
        ...
        it do
          ...
          expect(page).to have_text(existing_procurement_route.organisation_name)
        end
        ...
        describe 'Managing a procurement route' do
          ...
          context 'Summary panel' do
            ...
            let(:active_meters) do
              3.times do
                create(:gas_meter, active: true, procurement_route: existing_procurement_route, school: school)
              end
            end
            let(:inactive_meters) do
              2.times do
                create(:gas_meter, active: false, procurement_route: existing_procurement_route)
              end
            end
            let(:setup_data) { [active_meters, inactive_meters] }

            it { expect(page).to have_text('Active meters 3') }
            it { expect(page).to have_text('Inactive meters 2') }
            it { expect(page).to have_text('Associated schools 3') }
          end
          ...
          describe 'Editing a procurement route' do
            ...
            it { expect(page).to have_text("Edit #{existing_procurement_route.organisation_name}") }
            ...
            context 'and saving new data' do
              ...
              it { expect(page).to have_text('Procurement route was successfully updated') }
          ...
          describe 'Deleting a procurement route' do
            ...
            it { expect(page).to have_no_text(existing_procurement_route.organisation_name) }
            it { expect(page).to have_text('Procurement route was successfully deleted') }
          end
      ...
      context 'creating a new procurement route' do
        ...
        it { expect(page).to have_text('New procurement route') }
        ...
        context 'with invalid attributes' do
          ...
          it { expect(page).to have_text("Organisation name *\ncan't be blank") }
        end
        ...
        context 'with new valid attributes' do
          ...
          it { expect(page).to have_text('Procurement route was successfully created') }
