...
describe AmrUploadedReading, type: :system do
  ...
  describe 'viewing configuration summary' do
    ...
    context 'with row per day config' do
      ...
      it 'explains this to user' do
        expect(page).to have_text('One row per day, with all half-hourly periods in columns')
      end

      it 'says how many header rows' do
        expect(page).to have_text('1 header rows')
      end
      ...
      context 'with no header rows' do
        ...
        it 'says there are no header rows' do
          expect(page).to have_text('No header row')
        end
      end

      it 'explains the MPAN field' do
        expect(page).to have_text('The MPAN/MPRN to be in a column labelled ' + config.mpan_mprn_field)
      end

      it 'explains the date field' do
        expect(page).to have_text('The reading date to be in a column labelled ' + config.reading_date_field)
      end

      it 'explains the reading fields' do
        expect(page).to have_text('Reading fields to be in columns labelled')
      end

      context 'with a serial number config' do
        let!(:config) do
          create(:amr_data_feed_config, row_per_reading: false, msn_field: 'Serial', lookup_by_serial_number: true)
        end

        it 'explains the serial number field' do
          expect(page).to have_text('The Meter Serial Numbers in a column labelled ' + config.msn_field)
        end
      end
    end
    ...
    context 'with row per reading config' do
      ...
      it 'explains this to user' do
        expect(page).to have_text('One row per half hour reading')
      end
      ...
      it 'explains the reading field column' do
        expect(page).to have_text('A reading field column labelled ' + config.reading_fields.first)
      end
      ...
      context 'with a positional index' do
        ...
        it 'explains the index column' do
          expect(page).to have_text('A numbered half-hourly period in a column labelled SettlementTime, e.g. 1, 2, 3, 4')
        end
      end
      ...
      context 'with a separate reading time column' do
        ...
        it 'explains the reading time column' do
          expect(page).to have_text('The reading times to specified in a separate column labelled ReadingTime')
          expect(page).to have_text('The separate reading times to be formatted like this')
        end
      end
    end
    ...
    context 'with delayed reading config' do
      ...
      it 'explains this to user' do
        expect(page).to have_text('this configuration will adjust the dates in the uploaded file backwards by 1 day')
      end
    end
  end
  ...
  describe 'normal file format' do
    ...
    %w[.csv .xlsx .xls].each do |ext|
      context "previewing a valid #{ext} file" do
        ...
        it { expect(page).to have_no_text('We have identified a problem') }
        it { expect(page).to have_text('Data preview') }
        it { expect(page).to have_text('2200012767323') }
        it { expect(page).to have_text('2200012030374') }
        it { expect(page).to have_text('2200040922992') }
        ...
        context 'and uploading with the new loader' do
          ...
          it { expect(page).to have_text('Processing') }
          it { expect(page).to have_no_link('Upload another file') }
          ...
          context 'when complete' do
            ...
            it { expect(page).to have_no_text('Processing') }
    ...
    it 'produces an error message when translator raise error' do
      ...
      expect(AmrUploadedReading.count).to be 0
      expect(page).to have_text('Error:')
      expect(page).to have_text('bad file')
    end
    ...
    it 'is helpful if a very different format file is loaded' do
      ...
      click_on 'Preview'
      expect(page).to have_text(AmrReadingData::ERROR_NO_VALID_READINGS)
    end

    it 'is helpful if a dodgy date format file is loaded' do
      attach_file('amr_uploaded_reading[data_file]',
                  'spec/fixtures/amr_upload_data_files/banes-bad-example-date-file.csv')
      click_on 'Preview'
      expect(page).to have_text(AmrReadingData::ERROR_NO_VALID_READINGS)
    end
    ...
    it 'is helpful if a single dodgy date format is in the file loaded' do
      ...
      click_on 'Preview'
      expect(page).to have_text(AmrReadingData::WARNING_BAD_DATE_FORMAT)
    end

    it 'is helpful if a dodgy mpan format file is loaded' do
      attach_file('amr_uploaded_reading[data_file]',
                  'spec/fixtures/amr_upload_data_files/banes-bad-example-missing-and-invalid-mpan-file.csv')
      click_on 'Preview'
      expect(page).to have_text(AmrReadingData::WARNING_MISSING_MPAN_MPRN)
    end
    ...
    it 'is helpful if a invalid mpan format file is loaded' do
      attach_file('amr_uploaded_reading[data_file]',
                  'spec/fixtures/amr_upload_data_files/banes-bad-example-missing-and-invalid-mpan-file.csv')
      click_on 'Preview'
      expect(page).to have_text(AmrReadingData::WARNING_INVALID_NON_NUMERIC_MPAN_MPRN)
    end

    it 'is helpful if a reading is missing' do
      attach_file('amr_uploaded_reading[data_file]',
                  'spec/fixtures/amr_upload_data_files/banes-bad-example-missing-data-file.csv')
      click_on 'Preview'
      expect(page).to have_text(AmrReadingData::WARNING_MISSING_READINGS)
    end
  end
  ...
  describe 'bad sheffield file' do
    let!(:config) do
      create(:amr_data_feed_config,
             ...
             reading_fields: %w[kWh_1 kWh_2 kWh_3 kWh_4 kWh_5 kWh_6 kWh_7 kWh_8 kWh_9
                                kWh_10 kWh_11 kWh_12 kWh_13 kWh_14 kWh_15 kWh_16 kWh_17 kWh_18 kWh_19 kWh_20 kWh_21 kWh_22 kWh_23 kWh_24 kWh_25 kWh_26 kWh_27 kWh_28 kWh_29 kWh_30 kWh_31 kWh_32 kWh_33 kWh_34 kWh_35 kWh_36 kWh_37 kWh_38 kWh_39 kWh_40 kWh_41 kWh_42 kWh_43 kWh_44 kWh_45 kWh_46 kWh_47 kWh_48],
             column_separator: ',',
    ...
    it 'handles a wrong file format' do
      attach_file('amr_uploaded_reading[data_file]',
                  'spec/fixtures/amr_upload_data_files/example-bad-sheffield-file.csv')
      click_on 'Preview'
      expect(page).to have_text(AmrReadingData::WARNING_MISSING_READINGS)
    end
    ...
    it 'handles a wrong file format' do
      attach_file('amr_uploaded_reading[data_file]',
                  'spec/fixtures/amr_upload_data_files/example-bad-sheffield-proper-file.csv')
      click_on 'Preview'
      expect(page).to have_text(AmrReadingData::WARNING_READING_DATE_MISSING)
    end
  end
  ...
  describe 'with row per reading' do
    ...
    it 'handles a correct file format' do
      ...
      expect(page).to have_no_text('We have identified a problem')
      expect(page).to have_text('Data preview')
      expect(page).to have_text('1712423842469')
    ...
    it 'handles a wrong file format' do
      attach_file('amr_uploaded_reading[data_file]', 'spec/fixtures/amr_upload_data_files/example-sheffield-file.csv')
      click_on 'Preview'
      expect(page).to have_text(AmrReadingData::ERROR_UNABLE_TO_PARSE_FILE)
    end
  end
end
