...
describe 'Baseload anomaly report' do
  ...
  let(:meter) do
    create(:electricity_meter, data_source: create(:data_source), school: create(:school, school_group: school_group))
  end
  ...
  it 'displays the table' do
    ...
    expect(rows).to eq([
                         ['School Group', 'Admin', 'School', 'Meter', 'Meter Name', 'Meter System', 'Data Source',
                          'Reading Date', 'Previous Baseload', 'Baseload', 'Chart', 'Issues & Notes'],
                         [meter.school_group.name, meter.school_group&.default_issues_admin_user&.name,
                          meter.school.name, meter.mpan_mprn.to_s, meter.name, meter.t_meter_system, meter.data_source.name, reading_date.iso8601, '20', '0', 'Chart', '']
                       ])
  end
  ...
  it 'allows csv download' do
    ...
    expect(body).to \
      eq("School Group,Admin,School,Meter,Meter Name,Meter System,Data Source,Reading Date,Previous Baseload,Baseload,Chart,Issues,Notes\n" \
         "#{meter.school_group.name},#{meter.school_group&.default_issues_admin_user&.name},#{meter.school.name},#{meter.mpan_mprn},#{meter.name},#{meter.t_meter_system},#{meter.data_source.name},#{reading_date},20,0,#{analysis_school_advice_baseload_url(
           meter.school, host: 'example.com'
         )},0,0\n")
  end
end
