...
describe 'Manual Reads Report' do
  ...
  it 'displays the table' do
    expect(all('tr').map { |tr| tr.all('th, td').map(&:text) }).to \
      eq([['School Group', 'Admin', 'School', 'Meter', 'Meter Name', 'Meter Type', 'Supplier', 'Data Source',
  ...
  it 'allows csv download' do
    ...
    expect(body).to \
      eq('School Group,Admin,School,Meter,Meter Name,Meter Type,Supplier,Data Source,Last Validated Date,Issues,' \
         "Notes\n#{meter.school.school_group.name},Admin,#{meter.school.name},#{meter.mpan_mprn},#{meter.name},gas" \
         ",,,,0,0\n")
  end
end
