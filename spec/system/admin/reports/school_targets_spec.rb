...
describe 'school targets report', type: :system do
  ...
  it 'displays the report' do
    ...
    expect(page).to have_text('Listing 2 currently active school targets')
    expect(page).to have_text(school_target_1.school.name)
    expect(page).to have_text(school_target_1.electricity)
    expect(page).to have_text(school_target_1.gas)
    expect(page).to have_text(school_target_1.storage_heaters)
