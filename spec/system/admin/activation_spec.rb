...
RSpec.describe 'activation', type: :system do
  ...
  let!(:data_visible) { create(:school, name: 'Data visible', school_group: school_group, visible: true, data_enabled: false) }
  let!(:onboarding) { create(:school_onboarding, school: data_visible, project_group: create(:school_group, :project_group)) }
  ...
  it 'displays the table' do
    expect(all('tr').map { |tr| tr.all('th, td').map(&:text) }).to eq(
      [
        ...
        [school_group.name, onboarding.project_group.name, school_group.default_issues_admin_user.name, data_visible.name, '', '', '', '', '0', 'No', '', 'Meters'],
