...
RSpec.describe 'equivalence type management', type: :system do
  ...
  let!(:equivalence_calc)   do
    double(from_date: Date.parse('20200101'), to_date: Date.parse('20210101'), formatted_variables: { kwh: kwh_value },
           hide_preview?: false)
  end
  ...
  context 'allows the deletion equivalences with context types' do
    ...
    it 'only' do
      expect do
        click_on 'Delete'
      end.to change(EquivalenceType, :count).by(-1).and change(EquivalenceTypeContentVersion, :count).by(-1)
    end
    ...
    it 'and equivalences too' do
      ...
      expect(analytics).to receive(:front_end_convert).at_least(:once).with(:kwh, { month: -1 },
      ...
      expect(analytics).to receive(:front_end_convert).at_least(:once).with(:number_trees, { month: -1 },
                                                                            :electricity).and_return(
                                                                              {
      ...
      expect do
        Equivalences::GenerateEquivalences.new(school: school, analytics_class: analytics,
                                               aggregate_school: aggregate_school).perform
      end.to change(Equivalence, :count).by(1)
      expect do
        click_on 'Delete'
      end.to change(EquivalenceType,
                    :count).by(-1).and change(EquivalenceTypeContentVersion,
                                              :count).by(-1).and change(Equivalence, :count).by(-1)
    end
  end
end
