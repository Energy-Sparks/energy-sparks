...
RSpec.describe 'meter reports', :amr_validated_readings, type: :system do
  ...
  context 'when a meter has readings' do
    ...
    it 'links to a rich calendar view', :js do
      click_on(meter.name.to_s)
      expect(page).to have_text 'This report provides a calendar view of the validated meter readings'
  ...
  context 'when there are gaps in the meter readings' do
    ...
    it 'shows count of modified dates and gaps' do
      expect(page).to have_text 'Large gaps (last 2 years)'
      expect(page).to have_text 'Modified readings (last 2 years)'

      within '.gappy-dates' do
        expect(page).to have_text "15 days (#{(base_date + 1.day).to_fs(:es_short)} to #{(base_date + 15.days).to_fs(:es_short)})"
      end
