...
describe 'compare pages', :compare, type: :system do
  ...
  shared_examples 'a results page with unlisted schools' do |unlisted_count: 0|
    ...
    context 'with multiple excluded schools', if: unlisted_count > 1 do
      it {
        expect(page).to have_text("#{unlisted_count} schools could not be shown in this report as they do not have enough data to be analysed")
      }
    ...
    context 'with a single excluded school', if: unlisted_count == 1 do
      it {
        expect(page).to have_text('1 school could not be shown in this report as it does not have enough data to be analysed')
      }
  ...
  describe 'when choosing options' do
    ...
    let!(:school)          do
      create(:school, country: :scotland, postcode: 'EH99 1SP', school_group: school_group, funder: funder)
    end
    let!(:school_2)        { create(:school, country: :scotland, postcode: 'EH99 1SP', school_group: school_group_2) }
    let!(:school_3)        do
      create(:school, country: :scotland, school_group: create(:school_group, name: 'Not Public', public: false))
    end

    let(:benchmark_groups) do
      [{ name: 'Benchmark group name', description: 'Benchmark description',
         benchmarks: { baseload_per_pupil: 'Baseload per pupil' } }]
    end
        ...
        context 'Benchmark page' do
          ...
          context 'Changing options' do
            ...
            it_behaves_like 'a form filter', id: '#groups', school_group_list: ['Group 1', 'Group 2'],
                                             school_groups: ['Group 1', 'Group 2'], school_types_excluding: ['infant']
          end
          ...
          context 'Filtering all schools' do
            ...
            it_behaves_like 'a filter summary', school_types_excluding: ['infant'],
                                                school_groups: ['Group 1', 'Group 2']
          ...
          context 'results page' do
            ...
            context 'Changing options' do
              ...
              it_behaves_like 'an index page', tab: 'Choose groups'
              it_behaves_like 'a form filter', id: '#groups', school_groups: ['Group 1', 'Group 2'],
                                               school_types_excluding: ['infant']
            end
          end
        end
      end
  ...
  describe 'when redirecting from the old implementation' do
    context 'when user is requesting an old report', type: :request do
      ...
      context 'with school groups' do
        ...
        let(:old_compare_url) do
          "/compare/baseload_per_pupil?school_group_ids%5B%5D=#{school_group.id}&school_group_ids%5B%5D=#{school_group_2.id}&school_types%5B%5D=primary&school_types%5B%5D=secondary&search=groups"
        end
      ...
      context 'without school groups' do
        let(:old_compare_url) do
          '/compare/baseload_per_pupil?school_types%5B%5D=primary&school_types%5B%5D=secondary&search=groups'
        end
    ...
    describe 'when user is requesting an old benchmark page', type: :request do
      ...
      context '/benchmark' do
        ...
        context 'without school groups' do
          let(:old_benchmark_url) do
            '/benchmark?benchmark%5Bschool_types%5D%5B%5D=0&benchmark%5Bschool_types%5D%5B%5D=1&benchmark%5Bschool_types%5D%5B%5D=2&benchmark%5Bschool_types%5D%5B%5D=3&benchmark%5Bschool_types%5D%5B%5D=4&benchmark%5Bschool_types%5D%5B%5D=5&benchmark%5Bschool_types%5D%5B%5D=6&benchmark_type=baseload_per_pupil'
          end
