...
RSpec.describe 'activity category', type: :system do
  ...
  let!(:activity_category_1) do
    create(:activity_category, name: 'cat1', description: 'save some energy', featured: true)
  end
  let!(:activity_type_1_1) do
    create(:activity_type, activity_category: activity_category_1, key_stages: [ks1, ks2],
                           description: 'public descriptive text here')
  end
  ...
  context toggle_feature: :todos do
    let!(:programme_types) do
      if Flipper.enabled?(:todos)
        create_list(:programme_type, 4, :with_todos)
      else
        create_list(:programme_type, 4)
      end
    end
    ...
    context 'as a public user' do
      describe 'activity categories can be viewed' do
        ...
        it 'shows featured activity categories with at least 4 activities, plus Pupil activities and Programmes, but not Recommended section' do
          ...
          expect(page).to have_no_text(activity_category_3.name)
          expect(page).to have_no_text('Recommended')

          expect(page).to have_text('Pupil activities')
          expect(page).to have_text(activity_category_4.name)
        end

        it 'shows 4 programmes' do
          expect(page).to have_text('Programmes')
        ...
        it 'shows 4 activities' do
          ...
          expect(page).to have_text(activity_type_1_3.name)
          expect(page).to have_text(activity_type_1_4.name)

          expect(page).to have_no_text(activity_type_2_1.name)
          expect(page).to have_no_text(activity_type_3_1.name)
        end
        ...
        it 'links to category page, activity page and back' do
          ...
          expect(page).to have_text(activity_type_1_1.name)
          expect(page).to have_text('public descriptive text here')
