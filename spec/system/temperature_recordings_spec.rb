...
describe 'temperature recordings as school admin' do
  ...
  context 'as a pupil' do
    ...
    context 'when the site settings are turned off' do
      before(:each) do
        SiteSettings.create!(temperature_recording_months: [], electricity_price: 1, solar_export_price: 1, gas_price: 1, oil_price: 1)
      end
    ...
    context 'when temperature recoding is turned on' do
      ...
      before(:each) do
        SiteSettings.create!(temperature_recording_months: (1..12).map(&:to_s), electricity_price: 1, solar_export_price: 1, gas_price: 1, oil_price: 1)
        sign_in(user)
        visit root_path
