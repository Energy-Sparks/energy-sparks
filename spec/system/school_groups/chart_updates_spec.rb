...
describe 'school group chart settings' do
  ...
  shared_examples 'a working chart update page' do
    it { expect(page).to have_text(I18n.t('school_groups.chart_updates.index.group_chart_settings')) }
    ...
    it 'displays correct form options' do
      SchoolGroup.default_chart_preferences.each_key do |preference|
        expect(page).to have_text(I18n.t("school_groups.chart_updates.index.default_chart_preference.#{preference}"))
      end
    end
