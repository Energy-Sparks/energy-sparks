...
describe 'managing school times' do
  ...
  it 'allows setting of daily values and validates the inputs' do
    ...
    expect(page).to have_text('Please note that while we will immediately save any changes')

    fill_in 'monday-opening_time', with: ''
    click_on 'Save school times'
    expect(page).to have_text("Opening time can't be blank")
    ...
    expect(page).to have_text('School times have been updated')
  end
end
