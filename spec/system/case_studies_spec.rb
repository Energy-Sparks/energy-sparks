...
RSpec.describe 'case_studies', :include_application_helper do
  ...
  context 'when there is an existing case study' do
    ...
    it 'displays the index page' do
      expect(page).to have_text 'First Case Study'
    end

    it 'shows the expected link' do
      expect(page).to have_link(I18n.t('common.labels.download'),
                                href: case_study_download_path(case_study, locale: :en))
    end
    ...
    context 'a welsh download is not available' do
      ...
      it 'the link is to the english download' do
        expect(page).to have_link(I18n.t('common.labels.download', locale: :cy),
                                  href: case_study_download_path(case_study, locale: :en))
      end
    end
  end
  ...
  context 'when a welsh download is available' do
    ...
    it 'shows the welsh link' do
      expect(page).to have_link(I18n.t('common.labels.download', locale: :cy),
                                href: case_study_download_path(case_study, locale: :cy))
    end
  ...
  context 'when case study is not published' do
    ...
    it 'does not display the case study' do
      expect(page).to have_no_text(case_study.title)
    end
  end
  ...
  context 'with new page layout' do
    ...
    let!(:case_study) do
      create(:case_study, tags: 'one, two, three', image: fixture_file_upload('spec/fixtures/images/laptop.jpg'))
    end
    ...
    context 'when some case studies do not have images' do
      ...
      it 'shows both case studies text' do
        expect(page).to have_text(case_study.title)
        expect(page).to have_text(case_study_without_image.title)
      end

      it 'does not show any images' do
        expect(page).to have_no_css("img[src*='laptop.jpg']")
      end
    ...
    context 'when all case studies have images' do
      let!(:case_study_with_image) do
        create(:case_study, image: fixture_file_upload('spec/fixtures/images/pupils-jumping.jpg'))
      end
