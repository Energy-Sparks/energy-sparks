...
class OnboardingMailerPreview < ActionMailer::Preview
  ...
  def reminder_email
    OnboardingMailer.with(email: 'test@test.com',
                          school_onboardings: [SchoolOnboarding.incomplete.first],
                          locale: locale).reminder_email
  end

  def onboarded_email
    OnboardingMailer.with(school: School.visible.first,
                          users: School.visible.first.users.school_admin,
                          locale: locale).onboarded_email
  end
  ...
  def data_enabled_email
    OnboardingMailer.with(school: School.visible.first,
                          users: School.visible.first.users.school_admin,
                          target_prompt: true,
                          locale: locale).data_enabled_email
  end
