...
RSpec.describe ImportMailer, include_application_helper: true do
  describe '#import_summary' do
    ...
    it 'sends an email for ' do
      ...
      expect(attachments.body.raw_source).to eq("\"\",Area,Meter type,School,MPAN/MPRN,Data source,Procurement route,Last validated reading date,Admin meter status,Issues,Notes,Group admin name\r\nMeter with stale data,#{sheffield_school.school_group.name},Gas,Sheffield School,1,#{meter_1.data_source.name},,#{meter_1.last_validated_reading&.strftime('%d/%m/%Y')},\"\",0,0,Admin Two\r\nMeter with stale data,#{bath_school.school_group.name},Gas,Bath School,2,#{meter_2.data_source.name},,#{meter_2.last_validated_reading&.strftime('%d/%m/%Y')},\"\",0,0,Admin One\r\n")
    end
  end
end
