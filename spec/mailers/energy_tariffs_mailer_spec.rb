  ...
  describe '#group_admin_review_group_tariffs_reminder' do
    ...
    context "preferred locale is cy" do
      it 'sends group admins a review group tariffs reminder email' do
        ...
        expect(email.subject).to eq(I18n.t('energy_tariffs_mailer.group_admin_review_group_tariffs_reminder.subject', locale: :cy))
        expect(email.to).to eq([school_group_admin.email])
        expect(email.body.to_s).to include(I18n.t('energy_tariffs_mailer.group_admin_review_group_tariffs_reminder.mail_body.to_help', locale: :cy))
    ...
    context "preferred locale is en" do
      it 'sends group admins a review group tariffs reminder email' do
        ...
        expect(ActionMailer::Base.deliveries.count).to eq(1)
        email = ActionMailer::Base.deliveries.last
        expect(email.subject).to eq(I18n.t('energy_tariffs_mailer.group_admin_review_group_tariffs_reminder.subject', locale: :en))
        expect(email.to).to eq([school_group_admin.email])
        expect(email.body.to_s).to include(I18n.t('energy_tariffs_mailer.group_admin_review_group_tariffs_reminder.mail_body.to_help', locale: :en))
  ...
  describe '#school_admin_review_school_tariffs_reminder' do
    ...
    context "preferred locale is en" do
      it 'sends school admins a review school tariffs reminder email' do
        ...
        expect(ActionMailer::Base.deliveries.count).to eq(1)
        email = ActionMailer::Base.deliveries.last
        expect(email.subject).to eq(I18n.t('energy_tariffs_mailer.school_admin_review_school_tariffs_reminder.subject', locale: :en))
        expect(email.to).to eq([school_admin.email])
        expect(email.body.to_s).to include(I18n.t('energy_tariffs_mailer.school_admin_review_school_tariffs_reminder.mail_body.to_help', locale: :en))
    ...
    context "preferred locale is cy" do
      it 'sends school admins a review school tariffs reminder email' do
        ...
        expect(ActionMailer::Base.deliveries.count).to eq(1)
        email = ActionMailer::Base.deliveries.last
        expect(email.subject).to eq(I18n.t('energy_tariffs_mailer.school_admin_review_school_tariffs_reminder.subject', locale: :cy))
        expect(email.to).to eq([school_admin.email])
        expect(email.body.to_s).to include(I18n.t('energy_tariffs_mailer.school_admin_review_school_tariffs_reminder.mail_body.to_help', locale: :cy))
