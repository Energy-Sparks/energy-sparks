...
describe LocaleHelper do
  ...
  describe '.t_params' do
    it 'returns array of locale-specific fields' do
      expect(helper.t_params(%i[name description],
                             %i[en cy])).to eq(%i[name_en description_en name_cy description_cy])
    end
  end
  ...
  describe '.t_fuels_as_sentence' do
    before do
      I18n.backend.store_translations('cy',
                                      { common: { electricity: 'Trydan', gas: 'Nwy',
                                                  storage_heater: 'Gwresogydd storio' } })
    ...
    it 'formats 2 fuels' do
      expect(helper.t_fuels_as_sentence(%i[gas electricity])).to eq('gas and electricity')
    end

    it 'formats multiple fuels' do
      expect(helper.t_fuels_as_sentence(%i[electricity gas
                                           storage_heater])).to eq('electricity, gas, and storage heater')
    end

    it 'shows missing translations (might want to adjust this)' do
      expect { helper.t_fuels_as_sentence(%i[wibble gas]) }.to raise_error(I18n::MissingTranslationData)
    end
    ...
    it 'translates sentence' do
      I18n.with_locale(:cy) do
        expect(helper.t_fuels_as_sentence(%i[electricity gas
                                             storage_heater])).to eq('trydan, nwy, a gwresogydd storio')
      end
    end
  end
