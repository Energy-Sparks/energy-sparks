...
describe IssuesHelper do
  ...
  describe '.issue_type_icons' do
    ...
    context "hiding when no issues of type present" do
      let(:hide_empty) { true }
      it { puts subject }
      it { expect(subject).to_not have_css('i.fa-sticky-note.text-secondary') }
      it { expect(subject).to_not have_css('i.fa-exclamation-circle.text-danger') }
      it { expect(subject).to_not have_css("span") }
      context "issues includes a note" do
        before { school.issues << note }
        it { expect(subject).to have_css('i.fa-sticky-note.text-warning') }
        it { expect(subject).to_not have_css('i.fa-exclamation-circle.text-secondary') }
