...
describe AdvicePageHelper do
  ...
  describe '.compare_for_school_group_path' do
    ...
    it 'returns a path to the compare page for a specific benchmark_type and for a school with a school group' do
      ...
      expect(compare_for_school_group_path(:baseload_per_pupil, school)).to \
        eq("/compare/baseload_per_pupil?school_group_ids%5B%5D=#{school.school_group.id}")
    end
  end

  describe '.format_unit', :aggregate_failures do
    it 'formats co2 values' do
      expect(helper.format_unit(1_234_567, :co2)).to eq('1,234,567')
      expect(helper.format_unit(-4.736951571734001e-15, :co2)).to eq('0')
    end

    it 'formats kwh values' do
      expect(helper.format_unit(1_234_567, :kwh)).to eq('1,234,567')
      expect(helper.format_unit(-4.736951571734001e-15, :kwh)).to eq('0')
    end

    it 'formats pound values' do
