...
RSpec.describe AdviceRemindersComponent, :include_application_helper, :include_url_helpers, type: :component do
  ...
  context 'when rendering' do
    ...
    it {
      expect(html).to have_no_text(I18n.t('schools.school_targets.achieving_your_targets.view_energy_saving_opportunities'))
    }

    it {
      expect(html).to have_no_link(I18n.t('common.labels.view_opportunities'),
    ...
    context 'with alerts and priorities' do
      ...
      it {
        expect(html).to have_text(I18n.t('schools.school_targets.achieving_your_targets.view_energy_saving_opportunities'))
      }

      it {
        expect(html).to have_link(I18n.t('common.labels.view_opportunities'),
                                  href: priorities_school_advice_path(school))
      }
    end
  end
