...
RSpec.describe DashboardLearnMoreComponent, :include_url_helpers, type: :component do
  ...
  shared_examples 'a data enabled panel' do
    ...
    it { expect(html).to have_text(I18n.t('components.dashboard_learn_more.adult.title')) }
    it { expect(html).to have_text(I18n.t('components.dashboard_learn_more.adult.intro')) }
  ...
  context 'when school is data enabled' do
    ...
    context 'with solar pv' do
      ...
      it { expect(html).to have_text(I18n.t('components.dashboard_learn_more.adult.title_with_solar_pv')) }
    end
  end
  ...
  context 'when school is not data enabled' do
    ...
    it { expect(html).to have_text(I18n.t('schools.show.configuring_data_access')) }

    it 'does not link to the analysis' do
      expect(html).to have_no_link(I18n.t('common.explore_energy_data'),
                                   href: school_advice_path(school))
      expect(html).to have_no_link(I18n.t('components.dashboard_learn_more.adult.opportunities'),
                                   href: priorities_school_advice_path(school))
    end
