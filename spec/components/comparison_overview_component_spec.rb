...
RSpec.describe ComparisonOverviewComponent, :include_application_helper, :include_url_helpers, type: :component do
  ...
  describe '#can_benchmark_gas?' do
    ...
    context 'with gas but not enough data' do
      let(:school) do
        create(:school, :with_basic_configuration_single_meter_and_tariffs, fuel_type: :gas,
  ...
  context 'when rendering' do
    ...
    it { expect(html).to have_text(I18n.t("school_groups.clusters.group_type.#{school.school_group.group_type}")) }
    it { expect(html).to have_text(I18n.t('components.comparison_overview.title')) }
    ...
    it 'shows the fuel comparison' do
      ...
      expect(html).to have_text('How does your electricity use for the last 12 months compare to other Primary schools on Energy Sparks, with a similar number of pupils?')
    end
  end
end
