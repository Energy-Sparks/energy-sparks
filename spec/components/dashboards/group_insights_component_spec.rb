...
RSpec.describe Dashboards::GroupInsightsComponent, :include_application_helper, :include_url_helpers,
               type: :component do
  ...
  context 'when there are no alerts or reminders' do
    it { expect(html).to have_no_css('#group-reminders') }
    it { expect(html).to have_no_css('#group-alerts') }
  end
