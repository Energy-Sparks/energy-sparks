...
RSpec.describe Dashboards::GroupLearnMoreComponent, :include_application_helper, :include_url_helpers,
               type: :component do
  ...
  shared_examples 'a learn more component with a schools box' do
    it { expect(html).to have_text(I18n.t('schools.title', scope: i18n_scope)) }
    it { expect(html).to have_text(I18n.t('schools.intro', scope: i18n_scope)) }
    it { expect(html).to have_css("form[action='#{schools_path}'][method='get']") }
  ...
  shared_examples 'a learn more component with an advice box' do
    it { expect(html).to have_text(I18n.t('advice.title', scope: i18n_scope)) }
    it { expect(html).to have_text(I18n.t('advice.intro', scope: i18n_scope)) }
  ...
  shared_examples 'a learn more component without a data disabled box' do
    it { expect(html).to have_no_css('.data-disabled') }
    it { expect(html).to have_no_text(I18n.t('schools.show.coming_soon')) }
    it { expect(html).to have_no_text(I18n.t('intro_no_data', scope: i18n_scope)) }
  end
  ...
  context 'with only schools that are not data enabled' do
    ...
    context 'with data disabled box' do
      ...
      it { expect(html).to have_text(I18n.t('schools.show.coming_soon')) }
      it { expect(html).to have_text(I18n.t('intro_no_data', scope: i18n_scope)) }
    end

    context 'without advice box' do
      it { expect(html).to have_no_text(I18n.t('advice.title', scope: i18n_scope)) }
      it { expect(html).to have_no_text(I18n.t('advice.intro', scope: i18n_scope)) }

      it {
        expect(html).to have_no_link(I18n.t('common.explore_energy_data'), href: school_group_advice_path(school_group))
      }
    end
  end
  ...
  context 'without any schools available' do
    it 'does not render' do
      expect(html).to have_no_css('.dashboards-group-learn-more-component')
    end
  end
end
