...
RSpec.describe EnergyTariffTableComponent, type: :component do
  ...
  let(:energy_tariffs)    do
    [create(:energy_tariff, :with_flat_price, tariff_holder: tariff_holder, meter_type: :electricity, enabled: enabled,
                                              source: source)]
  end
  ...
  context 'action buttons' do
    ...
    context 'with unusable tariff' do
      # no prices
      let(:energy_tariffs) do
        [create(:energy_tariff, tariff_holder: tariff_holder, meter_type: :electricity, enabled: enabled,
                                source: source)]
      end
    ...
    context 'with non-authorised user' do
      ...
      it 'does not include the actions' do
        expect(page).to have_no_link(energy_tariffs.first.name)
