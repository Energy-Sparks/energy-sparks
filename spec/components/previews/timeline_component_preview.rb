class TimelineComponentPreview < ViewComponent::Preview
  ...
  def default(show_header: true, padding: true, show_actions: true, show_date: true, show_school: true,
    ...
    observations = [
      ...
      Observation.new(school: audit.school, observable: audit, observation_type: :audit_activities_completed,
                      at: Time.zone.yesterday, points: SiteSettings.current.audit_activities_bonus_points),
    ...
    render(TimelineComponent.new(observations: observations, show_header:, padding:,
                                 table_opts: { show_actions:, show_date:, show_school:, observation_style: }))
  end
end
