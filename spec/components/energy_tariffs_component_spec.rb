...
RSpec.describe EnergyTariffsComponent, type: :component do
  ...
  let!(:exp_solar_pv_tariffs) do
    create_list(:energy_tariff, 1, tariff_holder: tariff_holder, meter_type: :exported_solar_pv)
  end
  ...
  context 'when rendering tables' do
    it 'renders tables for selected tariff types' do
      ...
      expect(html).to have_no_css('#solar_pv-tariffs-table')
      expect(html).to have_no_css('#exported_solar_pv-tariffs-table')
    end
    ...
    context 'with smart meter tariffs' do
      ...
      context 'and no gas meters' do
        ...
        it 'adds correct message' do
          expect(html).to have_text(I18n.t('schools.user_tariffs.index.no_smart_meter_tariffs', meter_type: :gas))
        end
      end
    end
    ...
    context 'with default tariffs' do
      ...
      context 'and no gas meters' do
        ...
        it 'adds correct message' do
          expect(html).to have_text(I18n.t('schools.user_tariffs.index.no_defaults', meter_type: :gas))
        end
      end
      ...
      context 'and no gas meters' do
        ...
        it 'returns the usable and enabled site settings tariff for this meter type' do
          energy_tariff = EnergyTariff.create!(tariff_holder: SiteSettings.current, meter_type: 'gas',
                                               name: 'A site settings gas tariff', enabled: true)
          allow(EnergyTariff).to receive(:usable) { [energy_tariff] }
          expect(html).to have_text('A site settings gas tariff')
        end
      end
    end
  end
  ...
  context 'when rendering controls' do
    ...
    context 'when adding new tariffs is disabled' do
      ...
      it 'does not have the buttons' do
        expect(html).to have_no_link(I18n.t('schools.user_tariffs.index.electricity.add_label'))
        expect(html).to have_no_link(I18n.t('schools.user_tariffs.index.gas.add_label'))
      end
    end
  end
