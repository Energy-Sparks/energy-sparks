    ...
    if errors.present? && !example.metadata.key?(:errors_expected)
      aggregate_failures 'javascript errors' do
        errors.each do |error|
          # Repeated spec failures with 404s to embed urls generated by Google Font embed code:
          # "https://fonts.gstatic.com/s/quicksand/v37/[ID here].woff2 -
          # Failed to load resource: the server responded with a status of 404"
          if error.level == 'SEVERE' && error.message.include?('fonts.gstatic.com')
            warn 'IGNORED SEVERE (Google Font embed)'
            warn error.message
            next
          end
