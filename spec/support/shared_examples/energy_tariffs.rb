...
RSpec.shared_examples 'a tariff editor index' do
  ...
  it 'navigates the index tabs' do
    ...
    if tariff_holder.school?
      expect(page).to have_no_text('School tariffs')
      ...
      expect(page).to have_current_path("/school_groups/#{school_group.slug}/energy_tariffs/group_school_tariffs",
                                        ignore_query: true)
    end

    if tariff_holder.school? || tariff_holder.school_group?
      expect(page).to have_no_text('System default tariffs')
      click_link('User supplied tariffs')
      expect(current_path).to end_with('energy_tariffs')
    else
      expect(page).to have_no_text('User supplied tariffs')
    ...
    if tariff_holder.school?
      ...
      expect(page).to have_no_text('Smart meter tariffs')
    end
    ...
    if tariff_holder.school? || tariff_holder.school_group?
      ...
      expect(page).to have_no_text('Default tariffs')
    end
  end
  ...
  context 'when there are existing tariffs' do
    ...
    it 'displays the gas tariff' do
      within '#gas-tariffs-table' do
        expect(page).to have_text(gas_tariff.start_date.to_fs(:es_compact))
        expect(page).to have_text(gas_tariff.end_date.to_fs(:es_compact))
    ...
    it 'displays the electricity tariff' do
      within '#electricity-tariffs-table' do
        expect(page).to have_text(electricity_tariff.start_date.to_fs(:es_compact))
        expect(page).to have_text(electricity_tariff.end_date.to_fs(:es_compact))
...
RSpec.shared_examples 'the user can create a tariff' do
  it 'can create a new tariff' do
    expect(page).to have_text('Choose a name and date range')
    fill_in 'Name', with: 'My First Tariff'
    fill_in 'Start date', with: '15/08/2023'
    fill_in 'End date', with: '16/08/2023'
    click_button('Continue')

    expect(page).to have_text('My First Tariff')
    expect(page).to have_text('Dates')
    expect(page).to have_text('Start date')
    expect(page).to have_text('15/08/2023')
    expect(page).to have_text('End date')
    expect(page).to have_text('16/08/2023')
    expect(page).to have_text('Flat rate tariff')
    # Not yet usable
    expect(page).to have_text(I18n.t('schools.user_tariffs.show.not_usable'))
    expect(page).to have_no_css('#tariff-meters') unless tariff_holder.school?

    if current_user.admin?
      expect(page).to have_text('Notes (admin only)')
    else
      expect(page).to have_no_text('Notes (admin only)')
    end
    ...
    expect(page).to have_no_text('Add standing charges for this tariff')
    expect(page).to have_text('£1.11 per day')
    ...
    expect(energy_tariff.meters).to be_empty
...
RSpec.shared_examples 'the user can edit the tariff' do
  ...
  it 'allows me to edit the standing charge' do
    expect(page).to have_text('No standing charges have been added')
    find_by_id('charges-section-edit').click

    expect(page).to have_text('Add standing charges for this tariff')
    fill_in 'energy_tariff_charges[standing_charge][value]', with: '1.11'
    select 'day', from: 'energy_tariff_charges[standing_charge][units]'
    click_button('Continue')

    expect(page).to have_no_text('No standing charges have been added')
    expect(page).to have_text('£1.11 per day')
  end

  it 'allows me to edit the tariff metadata' do
    find_by_id('metadata-section-edit').click
    ...
    expect(page).to have_text('My Updated Tariff')
    expect(page).to have_text('Dates')
    expect(page).to have_text('Start date')
    expect(page).to have_text('13/08/2023')
    expect(page).to have_text('End date')
    expect(page).to have_text('14/08/2023')
  end
end
...
RSpec.shared_examples 'the user can change the type of tariff' do
  it 'allows switching between tariff types, deleting any consumption charges asscoated with the previous tariff type' do
    ...
    find_by_id('tariff-type-section-edit').click
    expect(page).to have_text('Is this a flat rate tariff?')
    expect(page).to have_text('Or a rate which varies by time of day (e.g. Day/Night tariff, Economy7)')
    click_button('Differential tariff')
    expect(page).to have_text('Differential tariff')
    ...
    expect(page).to have_text('Differential tariff')
    expect(page).to have_text('£0.50 per kWh')
...
RSpec.shared_examples 'the user can not select the meter system' do
  it 'can not select which meter system types a tariff applies to' do
    expect(page).to have_text("All #{energy_tariff.meter_type} meters")
    expect(energy_tariff.applies_to).to eq('both')

    find_by_id('meters-section-edit').click

    expect(page).to have_text('Select meters for this tariff')
    check('all_meters')
    expect(page).to have_no_text('both half-hourly and non half-hourly meters')
    expect(page).to have_no_text('half-hourly meters only')
    expect(page).to have_no_text('non half-hourly meters only')
...
RSpec.shared_examples 'the user can select the meter system' do
  it 'can select which meter system types a tariff applies to' do
    expect(page).to have_text('All electricity meters')
    expect(energy_tariff.applies_to).to eq('both')
    ...
    expect(page).to have_text('Select meters for this tariff')
    check('all_meters')
    expect(page).to have_text('both half-hourly and non half-hourly meters')
    expect(page).to have_text('half-hourly meters only')
    expect(page).to have_text('non half-hourly meters only')
...
RSpec.shared_examples 'the user can not see the meterless applies to editor' do
  it 'can select which meter system types a tariff applies to' do
    expect(page).to have_text("All #{energy_tariff.meter_type} meters")
    expect(page).to have_no_text('Tariff applies to')
  end
end
...
RSpec.shared_examples 'the meterless applies to editor' do
  ...
  it 'can select which meter system types an electricity tariff applies to' do
    ...
    expect(electricity_tariff.meters).to be_empty
    expect(electricity_tariff.applies_to).to eq('both')
    expect(page).to have_text('Tariff applies to')
    find_by_id('applies-to-section-edit').click
    expect(page).to have_text('Choose which meter systems this tariff applies to')
    expect(page).to have_text('both half-hourly and non half-hourly meters')
    expect(page).to have_text('half-hourly meters only')
    expect(page).to have_text('non half-hourly meters only')
    choose('energy_tariff[applies_to]', option: 'half_hourly')
  ...
  it 'can not select which meter system types a gas tariff applies to' do
    ...
    expect(gas_tariff.meters).to be_empty
...
RSpec.shared_examples 'the user can select the meters' do
  it 'can create a tariff and associate the meters' do
    ...
    expect(page).to have_no_text('All electricity meters')
    expect(page).to have_text(meter.name_or_mpan_mprn)
    energy_tariff.reload
  ...
  it 'requires a meter to be selected if we check the box' do
    ...
    expect(page).to have_text('Select meters for this tariff')
    expect(page).to have_text("Will this tariff apply to all #{meter.fuel_type} meters at the school or just specific meters?")
    uncheck('all_meters')
    uncheck(meter.mpan_mprn.to_s)

    click_button('Continue')
    expect(page).to have_text('Please select at least one meter for this tariff. Or uncheck option to apply tariff to all meters')
    expect(page).to have_text('Select meters for this tariff')
  end
end
