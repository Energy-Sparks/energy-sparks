...
RSpec.shared_examples 'an admin meter import report' do
  ...
  it 'allows csv download' do
    ...
    expect(body).to \
      eq("#{header.join(',')}\n" \
         "#{meter.school.school_group.name},Admin,#{meter.school.name},#{meter.mpan_mprn},#{meter.name},gas,#{meter.t_meter_system},#{meter.data_source&.name},,,N,#{end_date.to_date.iso8601},0,0\n")
  end
end
