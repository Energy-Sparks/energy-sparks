  ...
  describe Configuration do
    ...
    describe '#can_show_analysis_chart?' do
      it 'returns true if the chart is configured' do
        ...
        expect(configuration.can_show_analysis_chart?(:analysis_charts, :main_dashboard_electric,
                                                      :benchmark)).to eq(true)
      ...
      it 'handles nested sub_pages' do
        ...
        expect(configuration.can_show_analysis_chart?(:analysis_charts, :pupil_analysis_page, 'Nonsense',
                                                      :blah)).to eq(false)

        expect(configuration.can_show_analysis_chart?(:analysis_charts, :pupil_analysis_page, 'Gas',
                                                      :gas_dash)).to eq(true)
        expect(configuration.can_show_analysis_chart?(:analysis_charts, :pupil_analysis_page, 'Gas',
                                                      :blah)).to eq(false)

        expect(configuration.can_show_analysis_chart?(:analysis_charts, :pupil_analysis_page, 'Electricity', 'Solar',
                                                      :electric_solar)).to eq(true)
        expect(configuration.can_show_analysis_chart?(:analysis_charts, :pupil_analysis_page, 'Electricity', 'Solar',
                                                      :electric_storage)).to eq(false)

        expect(configuration.can_show_analysis_chart?(:analysis_charts, :pupil_analysis_page, 'Electricity', 'Storage',
                                                      :electric_storage)).to eq(true)

        expect(configuration.can_show_analysis_chart?(:analysis_charts, :pupil_analysis_page, 'Electricity', 'Lemons',
                                                      :electric_storage)).to eq(false)
      end
    end
  end
