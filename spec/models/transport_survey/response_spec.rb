...
describe TransportSurvey::Response do
  describe 'validations' do
    ...
    %i[transport_survey_id transport_type_id passengers run_identifier surveyed_at journey_minutes
       weather].each do |attribute|
  ...
  describe 'enums' do
    it { is_expected.to define_enum_for(:weather).with_values(%i[sun cloud rain snow]) }
  end
  ...
  describe '#carbon' do
    ...
    let(:carbon_calc) do
      ((transport_type.speed_km_per_hour * response.journey_minutes) / 60) * transport_type.kg_co2e_per_km
    end

    context 'when transport type is not park and stride' do
      let(:park_and_stride) { false }
      let(:response) do
        create(:transport_survey_response, transport_type: transport_type, passengers: 3, journey_minutes: 15)
      end
    ...
    context 'when transport type uses park and stride' do
      ...
      context "when journeys are #{TransportSurvey::Response.park_and_stride_mins} minutes and under" do
        let(:response) do
          create(:transport_survey_response, transport_type: transport_type, passengers: 3,
                                             journey_minutes: TransportSurvey::Response.park_and_stride_mins)
        end
      ...
      context "when journeys are over #{TransportSurvey::Response.park_and_stride_mins} mins" do
        let(:response) do
          create(:transport_survey_response, transport_type: transport_type, passengers: 3,
                                             journey_minutes: TransportSurvey::Response.park_and_stride_mins + 5)
        end
  ...
  describe '.to_csv' do
    ...
    let(:header) do
      'Id,Run identifier,Weather,Journey minutes,Transport type name,Passengers,Carbon kg co2,Surveyed at'
    end
    ...
    context 'with responses' do
      ...
      2.times do |i|
        it {
          expect(csv.lines[i + 1].chomp).to eq([responses[i].id, responses[i].run_identifier, responses[i].weather_name,
                                                responses[i].journey_minutes, responses[i].transport_type.name, responses[i].passengers, responses[i].carbon_kg_co2, responses[i].surveyed_at].join(','))
        }
      end
    end
