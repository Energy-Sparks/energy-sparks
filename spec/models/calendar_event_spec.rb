...
describe CalendarEvent do
  ...
  it 'sets its own academic year' do
    ...
    event_2 = create(:calendar_event_holiday, calendar: calendar, start_date: Date.new(2020, 1, 22), end_date: Date.new(2020, 1, 30))
  ...
  describe '#valid?' do
    ...
    describe 'date orders' do
      it 'is not valid when the end date and start date are in the wrong order' do
        expect(build(:calendar_event_holiday, calendar: calendar, start_date: Date.new(2018, 2, 2), end_date: Date.new(2018, 2, 1))).not_to be_valid
      end
    end
    ...
    describe 'overlapping event types' do
      describe 'for holidays' do
        it 'is not valid when there is another holiday with overlapping dates' do
          create(:calendar_event_holiday, calendar: calendar, start_date: Date.new(2018, 1, 22), end_date: Date.new(2018, 1, 30))
          expect(build(:calendar_event_holiday, calendar: calendar, start_date: Date.new(2018, 1, 23), end_date: Date.new(2018, 2, 1))).not_to be_valid
        end
        ...
        it 'is valid when the other does not overlap' do
          create(:term, calendar: calendar, start_date: Date.new(2018, 1, 22), end_date: Date.new(2018, 1, 30))
          expect(build(:calendar_event_holiday, calendar: calendar, start_date: Date.new(2018, 1, 31), end_date: Date.new(2018, 2, 1))).to be_valid
        end
        ...
        it 'is valid when the other is not of the right type' do
          ...
          expect(build(:calendar_event_holiday, calendar: calendar, start_date: Date.new(2018, 1, 12), end_date: Date.new(2018, 2, 1))).to be_valid
        end
      end
