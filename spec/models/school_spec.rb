...
describe School do
  ...
  it 'validates alternative heating percents' do
    %i[alternative_heating_oil_percent alternative_heating_lpg_percent alternative_heating_biomass_percent alternative_heating_district_heating_percent].each do |field|
      subject[field] = 100
      expect(subject).to be_valid
      subject[field] = 0
      expect(subject).to be_valid
  ...
  context 'school users' do
    ...
    it 'identifies different groups' do
      ...
      expect(subject.all_school_admins.sort_by(&:email)).to match_array([staff_2, school_admin, cluster_admin].sort_by(&:email))
      expect((subject.all_school_admins + subject.staff).sort_by(&:email)).to match_array([school_admin, cluster_admin, staff, staff_2, staff_2])
      expect(subject.all_adult_school_users.sort_by(&:email)).to match_array([school_admin, cluster_admin, staff, staff_2].sort_by(&:email))
    end
