...
describe School do
  ...
  describe '#minimum_reading_date' do
    it 'returns the minimum amr validated readings date if amr_validated_readings are present' do
      ...
      expect(subject.minimum_reading_date).to eq(base_date)
      expect(subject.minimum_reading_date).to eq(AmrValidatedReading.where(meter_id: meter.id).minimum(:reading_date))
    end
