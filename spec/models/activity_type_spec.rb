...
describe 'ActivityType' do
  ...
  context 'search by query term' do
    ...
    it 'finds activities by names with apostrophes' do
      ...
      expect(ActivityType.search(query: "Investigate whether the school's heating",
                                 locale: 'en')).to eq([activity_type_1])
    end
    ...
    it 'applies search variants' do
      ...
      expect(ActivityType.search(query: 'timing', locale: 'en')).to contain_exactly(activity_type_1, activity_type_2)
    end

    it 'finds search content for different locales' do
      I18n.locale = :en
      activity_type_1 = create(:activity_type, name: 'Starting the work', summary: 'is two',
                                               description: 'thirds of it')
      I18n.locale = :cy
  ...
  context 'scoped by key stage' do
    it 'filters activities by key stage' do
      ...
      expect(ActivityType.for_key_stages([key_stage_1])).to contain_exactly(activity_type_1, activity_type_3)
    end
  ...
  context 'scoped by subject' do
    it 'filters activities by subject' do
      ...
      expect(ActivityType.for_subjects([subject_1])).to contain_exactly(activity_type_1, activity_type_3)
    end
  ...
  context 'serialising for transifex' do
    context 'finding resources for transifex' do
      ...
      it '#tx_resources' do
        expect(ActivityType.tx_resources).to contain_exactly(activity_type_1)
      end
    end

    context 'when mapping fields' do
      let!(:activity_type) do
        create(:activity_type, name: 'My activity', description: 'description',
                               school_specific_description: 'Description {{#chart}}chart_name{{/chart}} {{#chart}}chart_name2|£{{/chart}}')
      end
      ...
      it 'maps all translated fields' do
        ...
        expect(data['en'][key].keys).to match_array(%w[name description_html school_specific_description_html
                                                       download_links_html summary])
      end

      it 'created categories' do
        expect(activity_type.tx_categories).to contain_exactly('activity_type')
      end
  ...
  context 'as transifex serialisable' do
    ...
    context 'when there are rewriteable links' do
      ...
      it 'correctly identifies rewriteable fields' do
        expect(ActivityType.tx_rewriteable_fields).to match_array(%i[description_cy school_specific_description_cy
                                                                     download_links_cy])
      end
