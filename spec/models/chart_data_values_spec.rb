...
describe ChartDataValues do
  ...
  let(:chart_data_values) do
    ChartDataValues.new(config, chart, transformations: transformations, allowed_operations: allowed_operations,
                                       drilldown_available: drilldown_available, parent_timescale_description: parent_timescale_description, y1_axis_choices: y1_axis_choices).process
  end
  ...
  describe '#series_translation_key_lookup' do
    it 'returns a hash matching series text keys to translation key values' do
      expect(chart_data_values.series_translation_key_lookup).to eq(
        { 'Degree Days' => 'degree_days', 'Temperature' => 'temperature', 'School Day Closed' => 'school_day_closed',
  ...
  describe '#label_point_and_max_for' do
    it 'returns the label, point format, and max value (if needed) for a given y2 data title' do
      expect(chart_data_values.send(:label_point_and_max_for,
                                    Series::Temperature::TEMPERATURE)).to eq(['°C', '{point.y:.2f} °C'])
      expect(chart_data_values.send(:label_point_and_max_for, 'Temperature')).to eq(['°C', '{point.y:.2f} °C'])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    Series::DegreeDays::DEGREEDAYS)).to eq(['<span>Degree<br />Days</span>',
                                                                            '{point.y:.2f} Degree Days'])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    'Degree Days')).to eq(['<span>Degree<br />Days</span>',
                                                           '{point.y:.2f} Degree Days'])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    Series::GridCarbon::GRIDCARBON)).to eq(['kg/kWh', '{point.y:.2f} kg/kWh', 0.5])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    'Carbon Intensity of Electricity Grid (kg/kWh)')).to eq(['kg/kWh',
                                                                                             '{point.y:.2f} kg/kWh', 0.5])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    Series::GasCarbon::GASCARBON)).to eq(['kg/kWh', '{point.y:.2f} kg/kWh', 0.5])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    'Carbon Intensity of Gas (kg/kWh)')).to eq(['kg/kWh', '{point.y:.2f} kg/kWh', 0.5])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    'Carbon Intensity')).to eq(['kg/kWh', '{point.y:.2f} kg/kWh', 0.5])
      expect(chart_data_values.send(:label_point_and_max_for, 'Carbon')).to eq(['kWh', '{point.y:.2f} kWh'])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    chart_data_values.translated_series_item_for(Series::Irradiance::IRRADIANCE))).to eq([
                                                                                                                           '<span>Brightness<br>of sunshine<br>W/m2</span>', '{point.y:.2f} W/m2'
                                                                                                                         ])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    'Solar Irradiance')).to eq(['<span>Brightness<br>of sunshine<br>W/m2</span>',
                                                                '{point.y:.2f} W/m2'])
      expect(chart_data_values.send(:label_point_and_max_for,
                                    'Solar')).to eq(['<span>Brightness<br>of sunshine<br>W/m2</span>',
                                                     '{point.y:.2f} W/m2'])
  ...
  describe '#trendline?' do
    it 'returns true if the data type starts with trendline' do
      expect(chart_data_values.send(:trendline?,
                                    :'trendline_heating_occupied_all_days =-138.9T + 2684, r2 = 0.64, n=138')).to be_truthy
      expect(chart_data_values.send(:trendline?,
                                    'trendline_heating_occupied_all_days =-138.9T + 2684, r2 = 0.64, n=138')).to be_truthy
      expect(chart_data_values.send(:trendline?,
                                    :'Trendline heating occupied all days =-138.9T + 2684, r2 = 0.64, n=138')).to be_truthy
      expect(chart_data_values.send(:trendline?,
                                    'Trendline heating occupied all days =-138.9T + 2684, r2 = 0.64, n=138')).to be_truthy
  ...
  describe '#reduced_trendline_series_data_for' do
    it 'merges elements of the x_axis with corresponding data elements replacing all but the maximum and minimum values with nil' do
      ...
      expect(chart_data_values.send(:reduced_trendline_series_data_for,
                                    [0, 1, 2, 3, 4, 5,
                                     6])).to eq([['Sunday', 0], ['Monday', nil], ['Tuesday', nil], ['Wednesday', nil], ['Thursday', nil], ['Friday', nil],
                                                 ['Saturday', 6]])
      expect(chart_data_values.send(:reduced_trendline_series_data_for,
                                    [0, 6, 2, -1, 4, 5,
                                     3])).to eq([['Sunday', nil], ['Monday', 6], ['Tuesday', nil], ['Wednesday', -1], ['Thursday', nil], ['Friday', nil],
                                                 ['Saturday', nil]])
    end
  end
  ...
  describe '#scatter_series_data_for' do
    it 'merges elements of the x_axis with corresponding data elements' do
      expect(chart_data_values.send(:scatter_series_data_for,
                                    [0, 1, 2, 3, 4, 5,
                                     6])).to eq([['Sunday', 0], ['Monday', 1], ['Tuesday', 2], ['Wednesday', 3], ['Thursday', 4], ['Friday', 5],
                                                 ['Saturday', 6]])
      expect(chart_data_values.send(:scatter_series_data_for,
                                    [6, 2, 5, 4, 3, 0,
                                     1])).to eq([['Sunday', 6], ['Monday', 2], ['Tuesday', 5], ['Wednesday', 4], ['Thursday', 3], ['Friday', 0],
                                                 ['Saturday', 1]])
    end
  end
  ...
  context 'with benchmark charts' do
    ...
    let(:chart_type) { :bar }
    let(:x_axis) do
      ['09 Feb 2019 to 07 Feb 2020', '08 Feb 2020 to 05 Feb 2021', 'Exemplar School', 'Benchmark (Good) School']
    end
