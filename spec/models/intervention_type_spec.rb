...
describe 'InterventionType' do
  ...
  context 'search by query term' do
    ...
    it 'applies search variants' do
      ...
      expect(InterventionType.search(query: 'timing',
                                     locale: 'en')).to contain_exactly(
                                       intervention_type_2, intervention_type_1
                                     )
    end

    it 'finds search content for different locales' do
      I18n.locale = :en
      intervention_type_1 = create(:intervention_type, name: 'Starting the work', summary: 'is two',
                                                       description: 'thirds of it')
      I18n.locale = :cy
      intervention_type_2 = create(:intervention_type, name: 'Deuparth gwaith', summary: 'yw ei',
                                                       description: 'ddechrau')
  ...
  context 'serialising for transifex' do
    context 'when mapping fields' do
      let!(:intervention_type) do
        create(:intervention_type, name: 'My intervention', description: 'description', summary: 'summary')
      end
