...
describe ScoredSchoolsList do
  describe '#with_points' do
    ...
    context 'when always_include is not set' do
      it 'returns just those with points' do
        expect(ScoredSchoolsList.new([school_a, school_b, school_c,
                                      school_d]).with_points).to contain_exactly(school_a, school_b)
      end
    end
    ...
    context 'when always_include is set school with 0 points' do
      it 'includes school' do
        expect(ScoredSchoolsList.new([school_a, school_b, school_c,
                                      school_d]).with_points(always_include: school_c)).to contain_exactly(school_c,
                                                                                                           school_a, school_b)
      end
    end
    ...
    context 'when always_include is set school with nil points' do
      it 'includes school' do
        expect(ScoredSchoolsList.new([school_a, school_b, school_c,
                                      school_d]).with_points(always_include: school_d)).to contain_exactly(school_d,
                                                                                                           school_a, school_b)
      end
    end
  end
  ...
  describe '#schools_with_positions' do
    ...
    context 'schools with the same points' do
      ...
      it 'orders the schools based on their index' do
        expect(ScoredSchoolsList.new([school_a, school_b, school_c, school_d, school_e,
                                      school_f]).schools_with_positions).to eq(
                                        {
                                          1 => [school_a],
                                          2 => [school_b, school_c],
                                          3 => [school_d],
