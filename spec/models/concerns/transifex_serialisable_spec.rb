...
describe TransifexSerialisable do
  ...
  context 'with real classes' do
    ...
    it 'model shows no content present if no attributes set' do
      ...
      expect(ActivityCategory.new).not_to have_content # rubocop:disable Capybara/RSpec/HaveContent -- calling has_content?
    end
    ...
    it 'model shows content present if some attribute set' do
      ...
      expect(serialisable).to have_content # rubocop:disable Capybara/RSpec/HaveContent -- calling has_content?
    end
  end
end
