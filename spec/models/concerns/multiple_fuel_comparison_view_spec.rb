...
describe MultipleFuelComparisonView do
  ...
  describe '#field_names' do
    it 'returns all expected attributes' do
      expect(model.field_names(period: :previous_period)).to match_array(%i[electricity_previous_period_kwh
                                                                            gas_previous_period_kwh storage_heater_previous_period_kwh])
      expect(model.field_names(period: :current_period)).to match_array(%i[electricity_current_period_kwh
                                                                           gas_current_period_kwh storage_heater_current_period_kwh])
      expect(model.field_names(period: :current_period,
                               unit: :co2)).to match_array(%i[electricity_current_period_co2 gas_current_period_co2
                                                              storage_heater_current_period_co2])
      expect(model.field_names(period: :current_period,
                               unit: :£)).to match_array(%i[electricity_current_period_gbp gas_current_period_gbp
                                                            storage_heater_current_period_gbp])
    end
    ...
    describe 'with customised_fuel_types' do
      ...
      it 'returns all expected attributes' do
        expect(model.field_names(period: :previous_period)).to match_array(%i[electricity_previous_period_kwh
                                                                              gas_previous_period_kwh])
        expect(model.field_names(period: :current_period)).to match_array(%i[electricity_current_period_kwh
                                                                             gas_current_period_kwh])
        expect(model.field_names(period: :current_period,
                                 unit: :co2)).to match_array(%i[
                                                               electricity_current_period_co2 gas_current_period_co2
                                                             ])
        expect(model.field_names(period: :current_period,
                                 unit: :£)).to match_array(%i[
                                                             electricity_current_period_gbp gas_current_period_gbp
                                                           ])
      end
    end
  end
