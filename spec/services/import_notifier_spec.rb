...
describe ImportNotifier do
  ...
  let!(:meter_1)         { create(:gas_meter_with_validated_reading_dates, :with_unvalidated_readings,
  ...
  describe '#meters_running_behind' do
    ...
    context 'and there are inactive meters' do
      ...
      let!(:inactive_meter) { create(:gas_meter_with_validated_reading_dates, :with_unvalidated_readings,
  ...
  describe '#meters_with_blank_data' do
    ...
    let(:meters_with_blank_data)  { ImportNotifier.new.meters_with_blank_data(from: 2.days.ago, to: Time.now) }
  ...
  describe '#meters_with_zero_data' do
    ...
    let(:meters_with_zero_data)  { ImportNotifier.new.meters_with_zero_data(from: 2.days.ago, to: Time.now) }
  ...
  describe '#notify' do
    ...
    before do
      ImportNotifier.new(description: description).notify(from: 2.days.ago, to: Time.now)
    end
    ...
    context 'with meter data' do
      let!(:bath_import_log) { create(:amr_data_feed_import_log, amr_data_feed_config: bath_amr_data_feed_config,
