...
RSpec.describe Schools::AdvicePageBenchmarks::GenerateBenchmarks, type: :service do
  ...
  describe '#generate!' do
    before do
      expect(Schools::AdvicePageBenchmarks::SchoolBenchmarkGenerator).to receive(:generator_for).with(
        advice_page: advice_page_1, school: school, aggregate_school: aggregate_school
      ).and_return(generator)
      expect(Schools::AdvicePageBenchmarks::SchoolBenchmarkGenerator).to receive(:generator_for).with(
        advice_page: advice_page_2, school: school, aggregate_school: aggregate_school
      ).and_return(generator)
