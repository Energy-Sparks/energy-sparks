...
RSpec.describe Schools::ContactFinder do
  ...
  context 'when contact is connected to user' do
    let!(:contact) do
      create(:contact, name: 'Contact 1', school: school, user: user, email_address: 'totall@different.org')
    end

    it 'finds the contact by association' do
  ...
  context 'when contact is not connected to user' do
    ...
    it 'finds the contact by email' do
      expect(service.contact_for(user)).to eq(contact)
    end
  end
  ...
  context 'when contact does not have user id or same email' do
    let!(:contact) do
      create(:contact, name: 'Contact 1', school: school, user: nil, email_address: 'totall@different.org')
    end
