...
describe Schools::GenerateDashboardChartConfiguration, type: :service do
  ...
  it 'generates all expected charts' do
    ...
    expect(charts).to match_array(%i[management_dashboard_group_by_week_electricity
                                     management_dashboard_group_by_week_gas management_dashboard_group_by_week_storage_heater management_dashboard_group_by_month_solar_pv])
  end
  ...
  context 'school has limited fuel types' do
    ...
    it 'only generates necessary charts' do
      ...
      allow(chart_data).to receive(:has_chart_data?).and_return(true)
      charts = service.generate
      expect(charts).to match_array(%i[management_dashboard_group_by_week_gas
