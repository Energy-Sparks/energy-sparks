...
RSpec.describe Schools::Advice::GasRecentChangesService, type: :service do
  ...
  let(:gas_aggregate_meter)   { double('gas-aggregated-meter', aggregate_meter?: true, amr_data: OpenStruct.new)}
  ...
  describe '#at_least_x_full_weeks_of_data?' do
    context 'with at least 2 weeks of data with week starting Sunday' do
      it 'returns true' do
        ...
        allow(gas_aggregate_meter).to receive(:amr_data) { OpenStruct.new(start_date: Date.parse('2023-03-19'), end_date: Date.parse('01-04-2023')) }
        expect(gas_aggregate_meter.amr_data.start_date.sunday?).to eq(true)
        expect(gas_aggregate_meter.amr_data.end_date.saturday?).to eq(true)
        expect(service.enough_data_for_full_week_comparison?).to eq(true)
      end

      it 'returns false' do
        # 05-04-2023 is a Wednesday
        allow(gas_aggregate_meter).to receive(:amr_data) { OpenStruct.new(start_date: Date.parse('2023-03-23'), end_date: Date.parse('05-04-2023')) }
        expect(gas_aggregate_meter.amr_data.start_date.thursday?).to eq(true)
        expect(gas_aggregate_meter.amr_data.end_date.wednesday?).to eq(true)
