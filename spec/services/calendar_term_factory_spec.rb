...
describe CalendarTermFactory do
  describe 'creates terms from hashes' do
    it 'creates full calendar with academic years' do
      ...
      create(:academic_year, start_date: Date.parse('2018-09-01'), end_date: Date.parse('2019-08-30'),
                             calendar: calendar)

      expect do
        CalendarTermFactory.new(calendar,
                                example_calendar_hash).create_terms
      end.to change(CalendarEvent, :count).by(holiday_count + term_count)
