...
RSpec.describe SchoolGroups::MeterReport do
  ...
  let(:header) do
    'School,Supply,Number,Meter,Half-Hourly,Data source,Admin meter status,Procurement route,Active,First validated reading,Last validated reading,Large gaps (last 2 years),Modified readings (last 2 years),Zero reading days'
  end
  ...
  describe '#csv_filename' do
    ...
    context 'when all_meters is true' do
      ...
      it {
        expect(meter_report.csv_filename).to eq("a-group-meter-report-#{frozen_time.iso8601.parameterize}-all-meters.csv")
      }
    end
  end
