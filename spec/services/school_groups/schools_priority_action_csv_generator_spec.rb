...
RSpec.describe SchoolGroups::SchoolsPriorityActionCsvGenerator do
  ...
  let!(:school_1)              do
    create(:school, school_group: school_group, number_of_pupils: 10, floor_area: 200.0, data_enabled: true,
                    visible: true, active: true)
  end
  let!(:school_2) do
    create(:school, school_group: school_group, number_of_pupils: 20, floor_area: 300.0, data_enabled: true,
                    visible: true, active: true)
  end
  ...
  context 'with school group data' do
    describe '#export' do
      it 'returns priority actions data as a csv for a school group' do
        csv = SchoolGroups::SchoolsPriorityActionCsvGenerator.new(schools: school_group.schools,
                                                                  alert_type_rating_ids: [alert_type_rating.id]).export
      ...
      context 'when including cluster' do
        ...
        it {
          expect(csv.lines[0]).to eq("Fuel,Description,School,Cluster,Number of pupils,Floor area (m2),Energy (kWh),Cost (£),CO2 (kg)\n")
        }

        context "when school doesn't have cluster" do
          it {
            expect(csv.lines[1]).to eq("Gas,Spending too much money on heating,#{school_group.schools.first.name},Not set,10,200.0,0,£1000,1100\n")
          }
        end

        context 'when school has a cluster' do
          let!(:cluster) do
            create(:school_group_cluster, school_group: school_group, name: 'My Cluster', schools: [school_1])
          end

          it {
            expect(csv.lines[1]).to eq("Gas,Spending too much money on heating,#{school_group.schools.first.name},My Cluster,10,200.0,0,£1000,1100\n")
          }
        end
      end
    end
