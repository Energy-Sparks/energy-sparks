...
describe Alerts::GenerateSubscriptionEvents do
  ...
  let!(:alert)                      do
    create(:alert, :with_run, alert_type: alert_type, school: school, rating: rating)
  end
  ...
  let(:weekly_alerts)               do
    school.alerts.joins(:alert_type).where(alert_types: { frequency: [:weekly] }).without_exclusions
  end
  ...
  context 'alerts and subscriptions' do
    ...
    let!(:alert_type_rating)        do
      create(:alert_type_rating, alert_type: alert.alert_type, rating_from: 1, rating_to: 6, sms_active: sms_active,
    ...
    let(:termly_alerts) do
      school.alerts.joins(:alert_type).where(alert_types: { frequency: [:termly] }).without_exclusions
    end
    let(:no_frequency_alerts) do
      school.alerts.joins(:alert_type).where(alert_types: { frequency: [] }).without_exclusions
    end
    ...
    context 'contacts with email, sms and both' do
      context 'with some content' do
        ...
        it 'does not process anything the frequency is set to empty' do
          expect { service.perform(no_frequency_alerts) }.not_to(change do
            subscription_generation_run.alert_subscription_events.count
          end)
        end

        it 'does not process anything the frequency is set to a different frequency' do
          expect { service.perform(termly_alerts) }.not_to(change do
            subscription_generation_run.alert_subscription_events.count
          end)
        end
        ...
        it 'creates events and associates the content versions' do
          expect { service.perform(weekly_alerts) }.to change {
            subscription_generation_run.alert_subscription_events.count
          }.by(4)
          ...
          expect(sms_and_email_contact.alert_subscription_events.pluck(:communication_type)).to match_array %w[sms
                                                                                                               email]
        end
        ...
        it 'does not create any events for the scope if there is an alert type exception' do
          ...
          expect { service.perform(weekly_alerts) }.not_to(change do
            subscription_generation_run.alert_subscription_events.count
          end)
        end

        it 'ignores if events already exist' do
          existing_event = AlertSubscriptionEvent.create!(alert: alert, contact: email_contact, status: :sent,
                                                          communication_type: :email, subscription_generation_run: SubscriptionGenerationRun.create(school: school), content_version: content_version)
        ...
        it 'ignores unsubscription records' do
          create(:alert_type_rating_unsubscription, contact: email_contact, alert_type_rating: alert_type_rating,
                                                    scope: :email)
