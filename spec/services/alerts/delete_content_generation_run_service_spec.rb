...
describe Alerts::DeleteContentGenerationRunService, type: :service do
  ...
  context 'when there are older runs to delete' do
    ...
    let(:content_generation_run_2) do
      create(:content_generation_run, school: school, created_at: older_than_date + 1.day)
    end
    ...
    let!(:dashboard_alert_2) do
      create(:dashboard_alert, alert: alert_2, content_version: content_version_1,
                               content_generation_run: content_generation_run_2)
    end
    let!(:management_priority_1) do
      create(:management_priority, alert: alert_1, content_generation_run: content_generation_run_1)
    end
    let!(:management_priority_2) do
      create(:management_priority, alert: alert_2, content_generation_run: content_generation_run_2)
    end
    let!(:management_dashboard_table_1) do
      create(:management_dashboard_table, alert: alert_1, content_generation_run: content_generation_run_1)
    end
    let!(:management_dashboard_table_2) do
      create(:management_dashboard_table, alert: alert_2, content_generation_run: content_generation_run_2)
    end
    ...
    it 'deletes only the older runs and all of the older runs dependent objects' do
      ...
      expect { service.delete! }.to change(ContentGenerationRun, :count).from(2).to(1) &
                                    ...
                                    not_change(
                                      AlertTypeRating.where(id: alert_type_rating_content_versions.map(&:alert_type_rating_id)), :count
                                    ) &
