...
describe Alerts::FrameworkAdapter do
  ...
  it 'uses the adapters to create an alert object from the returned reports' do
    expect(Alerts::FrameworkAdapter.new(alert_type: alert_type, school: school, analysis_date: gas_date,
                                        aggregate_school: aggregate_school).analyse.to_json).to eq alert_report.to_json
  end
end
