...
describe Alerts::GenerateSmsNotifications do
  ...
  it 'sends sms, one, per alert' do
    ...
    expect(send_sms_class).to receive(:new).with('EnergySparks alert: You need to do something!',
                                                 sms_contact.mobile_phone_number).and_return(send_sms_service).ordered
    expect(send_sms_service).to receive(:send).ordered
    expect(send_sms_class).to receive(:new).with('EnergySparks alert: You need to fix something!',
                                                 sms_contact.mobile_phone_number).and_return(send_sms_service).ordered
    expect(send_sms_service).to receive(:send).ordered

    Alerts::GenerateSubscriptionEvents.new(school,
                                           subscription_generation_run: subscription_generation_run).perform([alert_1,
                                                                                                              alert_2])
    Alerts::GenerateSmsNotifications.new(subscription_generation_run: subscription_generation_run,
                                         send_sms_service: send_sms_class).perform
  ...
  it 'creates an SMS record' do
    ...
    expect(send_sms_class).to receive(:new).with('EnergySparks alert: You need to do something!',
                                                 sms_contact.mobile_phone_number).and_return(send_sms_service)
