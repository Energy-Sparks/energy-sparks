  ...
  describe LowCarbonHubUpserter do
    ...
    context 'with no existing meters' do
      ...
      it 'creates amr readings' do
        ...
        amr_reading = low_carbon_hub_installation.meters.find_by(mpan_mprn: expected_solar_pv_mpan).amr_data_feed_readings.last
        expect(amr_reading.readings[0]).to eq('0.5')
        amr_reading = low_carbon_hub_installation.meters.find_by(mpan_mprn: expected_electricity_mpan).amr_data_feed_readings.last
        expect(amr_reading.readings[0]).to eq('0.5')
        amr_reading = low_carbon_hub_installation.meters.find_by(mpan_mprn: expected_exported_solar_pv_mpan).amr_data_feed_readings.last
        expect(amr_reading.readings[0]).to eq('0.5')
      end
