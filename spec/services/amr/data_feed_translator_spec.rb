...
describe Amr::DataFeedTranslator do
  context 'for a typical data file config' do
  ...
  context 'for a row per reading data file that splits the date and time into two columns' do
    ...
    it '' do
      result = Amr::DataFeedTranslator.new(corona_config, @readings).perform.first

      expect(result[:mpan_mprn]).to eq('Site Name')
      expect(result[:reading_date]).to eq('01/06/2023')
      expect(result[:description]).to eq('Corona Portal')
      expect(result[:readings].first).to eq('Site Name')
      expect(result[:readings].last).to eq('1.17700000')
      expect(result[:units]).to eq('kwh')
    end
  end
