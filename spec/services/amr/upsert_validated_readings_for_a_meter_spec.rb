  ...
  describe '#perform' do
    ...
    context 'when there are no validated readings in the database' do
      ...
      context 'and the analytics returns valid data' do
        ...
        let(:upload_datetime)   { DateTime.now }
      ...
      context 'and analytics returns invalid data' do
        let(:readings)    { 1 }
        let(:dashboard_meter)   { build(:dashboard_gas_meter_with_validated_reading,
          reading_count: readings, actual_meter: active_record_meter, kwh_data_x48: Array.new(48, Float::NAN)) }
    ...
    context 'when there are validated readings in the database' do
      ...
      context 'and the analytics has substituted the data' do
        let(:substitute_date)  { Date.today - 7 }
        let(:status)           { 'GSS1' }
        #force this to be very different so we can be sure we're saving right value
        let(:upload_datetime)  { DateTime.now - 10 }
        ...
        it 'updates the existing records' do
          ...
          expect(first_reading.status).to eq status
          expect(first_reading.substitute_date).to eq substitute_date
          expect(first_reading.upload_datetime.utc.to_s).to eq upload_datetime.utc.to_s
        end
      end

      context 'and the supplier has sent new data' do
        #Note: we're simulating supplier having sent new original data by
        #building readings for same date, same status but different kwh values
        let(:upload_datetime)  { DateTime.now - 10 }
        let(:updated_data)     { Array.new(48, 2.5) }
        let(:dashboard_meter)   { build(:dashboard_gas_meter_with_validated_reading,
            start_date: start_date,
            end_date: end_date,
            actual_meter: active_record_meter,
            status: status,
            kwh_data_x48: updated_data,
            upload_datetime: upload_datetime
          )
        }
        ...
        it 'updates the existing records' do
          ...
          expect(first_reading.reading_date).to eq start_date
          expect(first_reading.status).to eq status
          expect(first_reading.kwh_data_x48).to eq updated_data
          expect(first_reading.upload_datetime.utc.to_s).to eq upload_datetime.utc.to_s
        end
