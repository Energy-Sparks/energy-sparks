...
module Amr
  describe ProcessAmrReadingData do
    ...
    let(:reading_data_second) do
      { mpan_mprn: '123', reading_date: Date.parse('2019-01-02'), readings: Array.new(48, '0.0'),
        amr_data_feed_config_id: amr_data_feed_config.id }
    end
    let(:reading_data_third) do
      { mpan_mprn: '123', reading_date: Date.parse('2019-01-03'), readings: Array.new(48, '0.0'),
        amr_data_feed_config_id: amr_data_feed_config.id }
    end
    let(:reading_data_warning) do
      { mpan_mprn: '1234567890123', reading_date: Date.parse('2019-01-03'), readings: Array.new(40, '0.0'),
        amr_data_feed_config_id: amr_data_feed_config.id, warnings: [:missing_readings] }
    end
    ...
    describe 'creates a warning if required' do
      it 'creates a warning if required' do
        ...
        meter = create(:electricity_meter, school: school, mpan_mprn: 1_234_567_890_123)
        expect do
          service.perform(valid_reading_data,
                          [reading_data_warning])
        end.to change(AmrDataFeedReading, :count).by(3).and change(AmrReadingWarning, :count).by(1)
