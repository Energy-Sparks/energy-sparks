...
describe Benchmarking::BenchmarkContentGasOutOfHoursUsage, type: :service do
  let(:benchmark) do
    described_class.new(
      ...
      Benchmarking::BenchmarkManager::CHART_TABLE_CONFIG[:annual_gas_out_of_hours_use]
    )
  end

  describe '#page' do
    it 'returns a chart name if charts are present' do
      expect(benchmark.page_name).to eq(:annual_gas_out_of_hours_use)
    end
  end
  ...
  describe 'introduction_text' do
    it 'formats introduction and any caveat text as html' do
      ...
      HTML
      content_html = I18n.t('analytics.benchmarking.content.annual_gas_out_of_hours_use.introduction_text_html')
  ...
  describe '#chart_name' do
    it 'returns a chart name if charts are present' do
      expect(benchmark.send(:chart_name)).to eq(:annual_gas_out_of_hours_use)
    end
  end
