...
describe Benchmarking::BenchmarkStorageHeatersOnDuringHoliday, type: :service do
  ...
  describe '#page' do
    it 'returns a chart name if charts are present' do
      expect(benchmark.page_name).to eq(:storage_heater_consumption_during_holiday)
    end
  end
  ...
  describe 'introduction_text' do
    it 'formats introduction and any caveat text as html' do
      ...
      HTML
      content_html = I18n.t('analytics.benchmarking.content.storage_heater_consumption_during_holiday.introduction_text_html')
  ...
  describe '#chart_name' do
    it 'returns a chart name if charts are present' do
      expect(benchmark.send(:chart_name)).to eq(:storage_heater_consumption_during_holiday)
    end
  end
