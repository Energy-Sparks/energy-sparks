class MoveAuditObervations < ActiveRecord::Migration[6.0]
  def up
    add_column :observations, :message_type, :integer, default: 0 # enum - defaults to :default
    add_index :observations, :message_type
  ...
  def down
    Observation.where(observable_type: 'Audit', message_type: :default).find_each do |observation|
    ...
    Observation.where(observable_type: 'Audit', message_type: :completed).find_each do |observation|
      observation.audit = observation.observable
      observation.observable = nil
    ...
    remove_column :observations, :message_type
  end
end
