CSV.generate do |csv|
  ...
  @results.each do |result|
   csv << [
        ...
        format_unit(percent_change(result.previous_year_co2, result.current_year_co2) * 100,
                    Float, true, :benchmark),
        format_unit(result.previous_year_gbp, Float, true, :benchmark),
        format_unit(result.current_year_gbp, Float, true, :benchmark),
        format_unit(percent_change(result.previous_year_gbp, result.current_year_gbp) * 100,
                    Float, true, :benchmark),
