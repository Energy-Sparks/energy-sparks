CSV.generate do |csv|
  ...
  @results.each do |result|
    ...
    csv << [
      ...
      format_unit(percent_change(result.storage_heater_previous_period_kwh, result.storage_heater_current_period_kwh) * 100, Float),
      format_unit(result.storage_heater_previous_period_co2, Float),
      format_unit(result.storage_heater_current_period_co2, Float),
      format_unit(percent_change(result.storage_heater_previous_period_co2, result.storage_heater_current_period_co2) * 100, Float),
      format_unit(result.storage_heater_previous_period_gbp, Float),
      format_unit(result.storage_heater_current_period_gbp, Float),
      format_unit(percent_change(result.storage_heater_previous_period_gbp, result.storage_heater_current_period_gbp) * 100, Float)
    ].select(&:itself)
  end
end.html_safe
