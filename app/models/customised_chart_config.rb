...
class CustomisedChartConfig
  ...
  def customise_y_axis(new_config, overrides)
    if overrides[:y_axis_units]
      ...
      if valid_choices && valid_choices.include?(overrides[:y_axis_units])
        new_config[:yaxis_units] = overrides[:y_axis_units]
      end
    end
  end

  def customise_meter_definition(new_config, overrides)
    new_config[:meter_definition] = overrides[:mpan_mprn].to_i if overrides[:mpan_mprn].present?
  end

  def customise_series_breakdown(new_config, overrides)
    new_config[:series_breakdown] = overrides[:series_breakdown].to_sym if overrides[:series_breakdown].present?
  end
