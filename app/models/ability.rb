...
# :view_settings - view group settings page
# :read, :my_school_group_menu - see my school group menu
# :update_settings - can manage settings (chart prefs, clusters) for school group. But also used to gate
# access to viewing clusters on school group dashboard.
# :secr - Used to control access to SECR report page
#
# METERS
#
...
class Ability
  ...
  def initialize(user)
    ...
    if user.guest?
      ...
      onboarding_permissions(user)
    elsif user.admin? || user.analytics?
  ...
  def onboarding_permissions(user)
    return unless user.school_onboarding?
  ...
  def school_user_common_permissions(user, school_scope = { id: user.school_id, visible: true })
    ...
    can :manage, [Activity, Observation], school: school_scope
