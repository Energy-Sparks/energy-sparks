module TransifexSerialisable
  ...
  def tx_update(data, locale)
    ...
    tx_attributes.each_key do |attr|
      ...
      next unless translated_attributes.include?(attr)
  ...
  def tx_status
    TransifexStatus.find_by(model: self)
  end
  ...
  def tx_value(attr)
    ...
    if self.class.tx_rich_text_field?(attr)
      ...
      value = send("#{attr}_#{I18n.default_locale}".to_sym)
    end
    value = mustache_to_yaml(value) if self.class.tx_templated_attribute?(attr)
    value ? value.strip : value
  end

  def resource_key
    "#{self.class.model_name.i18n_key}_#{id}"
  end
  ...
  module ClassMethods
    ...
    def tx_html_attribute?(attr)
      ...
      mapping.key?(:html) && mapping[:html]
    end

    def tx_rewrite_links?(attr)
      tx_model_has_link_rewrites? && tx_rewriteable_fields.include?(attr)
    end

    def tx_templated_attribute?(attr)
      mapping = tx_attribute_mapping(attr)
      mapping.key?(:templated) && mapping[:templated]
    end
