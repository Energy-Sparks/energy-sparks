...
class AmrDataFeedConfig < ApplicationRecord
  ...
  def self.set_up_sheffield
    if where(description: 'Sheffield', access_type: 'Email').exists?
      find_by(description: 'Sheffield', access_type: 'Email')
    else
      self.create(
        area_id: 3,
