...
class MeterAttributes
  class HeatingNonHeatingDaySeparationModelOverride < MeterAttributeTypes::AttributeBase
    ...
    structure MeterAttributeTypes::Symbol.define(required: true,
                                                 allowed_values: %i[fixed_single_value_temperature_sensitive_regression_model temperature_sensitive_regression_model
                                                                    temperature_sensitive_regression_model_covid_tolerant no_idea either not_enough_data])
  end
end
