...
class AggregateDataServiceSolar
  ...
  def process_solar_pv_electricity_meter(pv_meter_map)
    Rails.logger.debug { "Aggregation service: processing mains meter #{pv_meter_map[:mains_consume]} with solar pv" }
  ...
  def print_meter_map(pv_meter_map)
    Rails.logger.debug { 'PV Meter map:' }
    pv_meter_map.each do |meter_type, meter|
      if meter.is_a?(Array)
        Rails.logger.debug { "    #{meter_type} => #{meter.map { |m| format_meter(m) }.join('; ')}" }
      else
        Rails.logger.debug { "    #{format('%-25.25s', meter_type)} -> #{format_meter(meter)}" }
      end
    end
  end
  ...
  def print_final_setup(meter)
    Rails.logger.debug { "Final meter setup for #{meter} total #{meter.amr_data.total.round(0)}" }
    meter.sub_meters.each do |meter_type, sub_meter|
      Rails.logger.debug { "    sub_meter: #{meter_type} => #{sub_meter} name #{sub_meter.name} total #{sub_meter.amr_data.total.round(0)}" }
    end
  end
  ...
  def reference_as_sub_meter_for_subsequent_aggregation(mains_electricity_meter)
    Rails.logger.debug { "Referencing mains consumption meter #{mains_electricity_meter.mpan_mprn} without pv as sub meter for subsequent aggregation" }
  ...
  def create_export_meter(pv_meter_map)
    ...
    Rails.logger.debug { "Creating empty export meter between #{date_range.first} and #{date_range.last}" }
  ...
  def create_and_calculate_self_consumption_meter(pv_meter_map)
    Rails.logger.debug { 'Creating and calculating self consumption meter' }
  ...
  def aggregate_multiple_generation_meters(pv_meter_map)
    Rails.logger.debug { 'Aggregating multiple solar pv generation meters' }

    generation_meters = pv_meter_map.select { |type, meter| SolarMeterMap.generation_meters.include?(type) && !meter.nil? }.values

    Rails.logger.debug { "Aggregating these generation meters #{generation_meters.map(&:to_s).join(' + ')}" }
    ...
    Rails.logger.debug { "Created aggregate generation meter #{generation_meter} #{generation_meter.amr_data.total.round(0)}" }
  ...
  def normalise_date_ranges_of_sub_meters(pv_meter_map)
    ...
    Rails.logger.debug { "Reducing sub meter date ranges to between #{start_date} and #{end_date}" }
    meters.each do |meter|
  ...
  def truncate_to_mains_meter_dates(pv_meter_map)
    ...
    pv_meter_map.each do |meter_type, meter|
      ...
      Rails.logger.debug { "Mains meter start date #{mains_electricity_meter.amr_data.start_date} pv meter  #{meter.mpan_mprn} #{meter.fuel_type} start date #{meter.amr_data.start_date}" }
      ...
      if meter.amr_data.start_date < mains_electricity_meter.amr_data.start_date
        Rails.logger.debug { "Truncating meter #{meter.mpan_mprn} to start date of electricity meter #{mains_electricity_meter.amr_data.start_date}" }
      ...
      if meter.amr_data.end_date > mains_electricity_meter.amr_data.end_date
        Rails.logger.debug { "Truncating meter #{meter.mpan_mprn} to end date of electricity meter #{mains_electricity_meter.amr_data.end_date}" }
  ...
  def create_solar_pv_sub_meters_using_sheffield_pv_estimates(pv_map)
    Rails.logger.debug { 'Creating solar PV data from Sheffield PV feed' }
  ...
  def backfill_meter_with_zeros_deprecated(meter, mains_meter_start_date, _mpan_mapping_start_date)
    ...
    Rails.logger.debug { "Backfilling pv meter #{meter.mpan_mprn} with zeros between #{mains_meter_start_date} and #{meter.amr_data.start_date}" }
  ...
  def backfill_meter_with_zeros(meter, start_date, end_date)
    ...
    Rails.logger.debug { "Backfilling pv meter #{meter.mpan_mprn} with zeros between #{start_date} and #{end_date}" }
  ...
  def create_export_self_consumption_where_real_generation_data_but_no_export(pv_meter_map)
    ...
    if @apply_scaling_factor_for_debug
      ...
      Rails.logger.debug { "Scaling data by a factor of #{scale_factor} " }
  ...
  def truncate_start_date(meter, start_date)
    ...
    if start_date < meter.amr_data.start_date
      Rails.logger.debug { "Error: solar_pv_mpan_meter_mapping meter attribute start_date #{start_date} < meter start_date #{meter.amr_data.start_date}" }
    elsif start_date > meter.amr_data.end_date
      Rails.logger.debug { "Error: solar_pv_mpan_meter_mapping meter attribute start_date #{start_date} > meter end_date #{meter.amr_data.end_date}" }
    else
      Rails.logger.debug { "Warning: overriding amr start date to #{start_date} for meter #{meter.mpan_mprn}" }
      meter.amr_data.set_start_date(start_date)
    end
  end
  ...
  def truncate_end_date(meter, end_date)
    ...
    if end_date > meter.amr_data.end_date
      Rails.logger.debug { "Error: solar_pv_mpan_meter_mapping meter attribute end_date #{end_date} > meter end_date #{meter.amr_data.end_date}" }
    elsif end_date < meter.amr_data.start_date
      Rails.logger.debug { "Error: solar_pv_mpan_meter_mapping meter attribute end_date #{end_date} < meter start_date #{meter.amr_data.start_date}" }
    else
      Rails.logger.debug { "Warning: overriding amr end date to #{end_date} for meter #{meter.mpan_mprn}" }
      meter.amr_data.set_end_date(end_date)
    end
  end
end
