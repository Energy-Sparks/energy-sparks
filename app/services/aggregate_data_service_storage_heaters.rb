...
class AggregateDataServiceStorageHeaters
  ...
  def disaggregate
    ...
    Rails.logger.debug { "Disaggregating storage heater meters for #{@meter_collection.name}: #{@electricity_meters.length} electricity meters" }
    ...
    Rails.logger.debug { "Disaggregation of storage heater meters for #{@meter_collection.name} complete in #{bm.round(3)} seconds" }
    Rails.logger.debug { '=' * 100 }
  end
  ...
  def summarise_aggregated_meter
    Rails.logger.debug { 'Aggregated Meter Setup' }
    Rails.logger.debug { "    appliance: #{aggregate_meter_description(@meter_collection.aggregated_electricity_meters)}" }
    Rails.logger.debug { "    storage:   #{aggregate_meter_description(@meter_collection.storage_heater_meter)}" }
    Rails.logger.debug { "    original:  #{aggregate_meter_description(@meter_collection.aggregated_electricity_meters.sub_meters[:mains_consume])}" }
  end
  ...
  def summarise_component_meters
    Rails.logger.debug('Component Meter Setup')
    @meter_collection.electricity_meters.each.with_index do |meter, i|
      Rails.logger.debug { "    Meter #{i}" }
      Rails.logger.debug { format('        %-18.18s %s', 'ex storage heater', meter_description(meter)) }
      Rails.logger.debug { format('        %-18.18s %s', 'original',          meter_description(meter.sub_meters[:mains_consume])) }
      Rails.logger.debug { format('        %-18.18s %s', 'storage heaters',   meter_description(meter.sub_meters[:storage_heaters])) }
    end
  end
