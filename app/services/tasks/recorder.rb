...
module Tasks
  class Recorder
    ...
    def self.new(recording, user)
      ...
      unless SUPPORTED_MODELS.include?(subclass_name)
        raise ArgumentError,
              "Unsupported recording type: #{recording.class.name}"
      end
    ...
    def process # rubocop:todo Naming/PredicateMethod
      if @recording.save(**(@user ? { context: :require_description } : {}))
        after_save
