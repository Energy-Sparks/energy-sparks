    ...
    def perform(subscription_frequency: [])
      ActiveRecord::Base.transaction do
        ContentGenerationRun.create!(school: @school).tap do |content_generation_run|
          latest_alerts = @school.latest_alerts_without_exception
          latest_alerts_with_frequency = @school.alerts.joins(:alert_type).where(alert_types: { frequency: subscription_frequency }).without_exception.latest
