...
module Schools
  class Remover
    ...
    def remove_school!
      raise Schools::Remover::Error, 'Cannot remove school while it is still visible' if @school.visible?

      @school.transaction do
        delete_school_issues if @school.update!({ active: false, process_data: false }.merge(inactive_dates))
        @school&.school_group&.touch # rubocop:disable Rails/SkipsModelValidations
      end
      SchoolArchivedMailer.archived(@school) if @archive
    end

    def reenable_school!
      raise Schools::Remover::Error, 'Cannot reenable an active school' if @school.active?
    ...
    def remove_users! # rubocop:todo Metrics/AbcSize, Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity
      raise Schools::Remover::Error, 'Cannot remove users while school is still visible' if @school.visible?
    ...
    def remove_meters!
      raise Schools::Remover::Error, 'Cannot remove meters while school is still visible' if @school.visible?
