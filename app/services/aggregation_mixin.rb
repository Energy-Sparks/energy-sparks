...
module AggregationMixin
  ...
  def aggregate_amr_data(meters, type, ignore_rules = false)
    if meters.length == 1
      Rails.logger.info 'Single meter, so aggregation is a reference to itself not an aggregate meter'
    ...
    Rails.logger.info "Aggregating data between #{min_date} and #{max_date}"
  ...
  def calculate_costs_for_meter(meter)
    Rails.logger.info "Creating economic & accounting costs for #{meter.mpan_mprn} fuel #{meter.fuel_type} from #{meter.amr_data.start_date} to #{meter.amr_data.end_date}"
    meter.set_tariffs
  end
