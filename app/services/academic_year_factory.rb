class AcademicYearFactory
  def initialize(calendar_area, start_date: '01-09', end_date: '31-08')
  ...
  def create(start_year: 1990, end_year: 2023)
    ...
    (start_year..end_year).each do |year|
      AcademicYear.where(calendar_area: @calendar_area, start_date: Date.parse("#{@start_date}-#{year}"), end_date: "#{@end_date}-#{year + 1}").first_or_create!
    end
