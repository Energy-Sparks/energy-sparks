module Database
  class EnergyTariffMigrationService
    ...
    def self.migrate_global_solar_meter_attributes
      solar_types = %i[solar_pv exported_solar_pv]
      ActiveRecord::Base.transaction do
    ...
    def self.migrate_school_group_accounting_tariffs(school_group)
      ...
      school_group.meter_attributes.where(
        attribute_type: %w[accounting_tariff accounting_tariff_differential]
    ...
    def self.meter_type(attribute)
      ...
      if meter_types.include?('exported_solar_pv') || meter_types.include?('solar_pv_exported_sub_meter')
        return :exported_solar_pv
      end
    ...
    def self.time_for_rate_type_period(rates, rate_type = :daytime_rate, range = :from)
      return nil if rates[rate_type].blank?
      return nil if rates[rate_type][range].blank?
