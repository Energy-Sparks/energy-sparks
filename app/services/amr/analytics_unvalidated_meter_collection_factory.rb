...
module Amr
  class AnalyticsUnvalidatedMeterCollectionFactory
    ...
    def build
      ...
      add_meters_and_amr_unvalidated_data
    end

  private

    def add_meters_and_amr_unvalidated_data
      @active_record_school.meters_with_readings(:gas).map do |active_record_meter|
        dashboard_meter = add_unvalidated_amr_data(active_record_meter)
        @meter_collection.add_heat_meter(dashboard_meter)
      end

      @active_record_school.meters_with_readings(:electricity).map do |active_record_meter|
        dashboard_meter = add_unvalidated_amr_data(active_record_meter)
    ...
    def add_unvalidated_amr_data(active_record_meter)
