...
module Amr
  class UpsertValidatedReadingsForAllMeters
    NAN_READINGS = Array.new(48, Float::NAN).freeze

    def initialize(dashboard_meters)
      @dashboard_meters = Array(dashboard_meters)
    end
    ...
    def perform # rubocop:disable Metrics/AbcSize
      ...
      @dashboard_meters.each do |dashboard_meter|
        Rails.logger.info "Processing: mpan_mprn: #{dashboard_meter.mpan_mprn} id: #{dashboard_meter.external_meter_id}"
        amr_data = dashboard_meter.amr_data

        validated_amr_data = amr_data.delete_if { |_reading_date, one_day_read| nan?(one_day_read) }
        next if validated_amr_data.empty?

        all_values.concat(convert_to_hash(dashboard_meter, validated_amr_data))

        Rails.logger.info "Prepared: #{dashboard_meter}"
        dashboard_meter.amr_data = validated_amr_data
      end

      return if all_values.empty?

      do_upsert(all_values)
    ...
    def create_custom_upsert(values)
      ...
      insert_all = ActiveRecord::InsertAll.new(
        ...
        values,
        on_duplicate: :update,
        unique_by: %i[meter_id reading_date],
        returning: false
      )
    ...
    def convert_to_hash(dashboard_meter, validated_amr_data)
