class DailyRegenerationJob < ApplicationJob
  ...
  def perform(school:)
    ...
    run_validate_and_persist_for(school)
    run_aggregate_school_services_for(school)
    run_content_batch_for(school)
    GoodJob.logger.info("#{DateTime.now.utc} Regeneration for school #{school.name} end")
  end

  def run_aggregate_school_services_for(school)
  ...
  def run_validate_and_persist_for(school)
