...
$(document).ready(function() {
  ...
  if ($(".charts").length > 0) {
    ...
    if (minMaxReadings.max.isoWeekday() == 6) { // i.e. Saturday, end of week
      var latestFullWeekStart = minMaxReadings.max.clone().subtract(6, 'days');
      var latestFullWeekEnd = minMaxReadings.max;
    } else {
      var latestSunday = minMaxReadings.max.clone().startOf('week');
      var latestFullWeekStart = latestSunday.clone().subtract(7, 'days');
      var latestFullWeekEnd = latestSunday.clone().subtract(1, 'days');
    }

    if ($('#datetimepicker1').length) {
      setUpDatePicker('#datetimepicker1', 'input#first-date-picker', {min: minMaxReadings.min, max: latestFullWeekEnd}, latestFullWeekStart);
    }
    if ($('#datetimepicker2').length) {
      setUpDatePicker('#datetimepicker2', 'input#comparison-date-picker', {min: minMaxReadings.min, max: latestFullWeekEnd}, latestFullWeekStart.clone().subtract(7, 'days'));
    }
    if ($('#datetimepicker3').length) {
      setUpDatePicker('#datetimepicker3', 'input#week-picker',  minMaxReadings, latestFullWeekStart);
    }
