class ReportsController < AdminController
  ...
  def cache_report
    ...
    @cache_report = @cache_keys.map do |key|
      ...
      minutes_left = ((Time.at.utc(expiry_time) - Time.now.utc) / 1.minute).round
      { key: key, created_at: Time.at.utc(created_at), expiry_time: Time.at.utc(expiry_time), minutes_left: minutes_left }
    end
  end
