  ...
  class SecrController < ApplicationController
    ...
    def index
      ...
      respond_to do |format|
        format.html
        format.csv do
          send_data csv_report(params[:csv]), filename: EnergySparks::Filenames.csv("secr-#{params[:csv]}")
        end
      end
    end
      ...
      CSV.generate do |csv|
        ...
        @meters.public_send(type == 'gas' ? :gas : :electricity)
          ...
          csv << [
            ...
            ((9..12).to_a + (1..8).to_a).map do |m|
              [summary.consumption[m]&.round(2),
               summary.quality[m]&.[](0)&.upcase]
            end,
