...
module Schools
  class ActivitiesController < ApplicationController
    ...
    def show
      ...
      @activity_type_content = if @activity.activity_type.data_driven? && !@school.data_enabled?
                                 interpolator.interpolate(:description).description
                               else
                                 interpolator
                                   .interpolate(:school_specific_description_or_fallback)
    ...
    def new
      ...
      @activity.activity_type = ActivityType.find(params.expect(:activity_type_id))
    ...
    def activity_params
      params.expect(activity: %i[school_id activity_type_id title description happened_on content
                                 pupil_count])
    end
    ...
    def activity_type
      @activity_type ||=
        if params[:activity_type_id].present?
          ActivityType.find(params.expect(:activity_type_id))
