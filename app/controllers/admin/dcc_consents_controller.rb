module Admin
  class DccConsentsController < AdminController
    def index
      @dcc_consent_calcs = if params[:sandbox]
                             Meters::DccConsentCalcs.new(Meter.dcc, production_data_api.list + sandbox_data_api.list)
                           else
                             Meters::DccConsentCalcs.new(Meter.dcc.reject(&:sandbox), production_data_api.list)
                           end
    end

    def grant
      meter = Meter.find_by(mpan_mprn: params[:mpxn])
