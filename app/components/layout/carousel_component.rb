module Layout
  class CarouselComponent < ApplicationComponent
    renders_many :panels, types: {
      equivalence: { renders: ->(**kwargs) { EquivalenceComponent.new(**add_classes(**kwargs)) }, as: :equivalence },
      grid: { renders: ->(**kwargs) { Layout::GridComponent.new(**add_classes(**kwargs)) }, as: :grid }
    }

    def add_classes(**kwargs)
