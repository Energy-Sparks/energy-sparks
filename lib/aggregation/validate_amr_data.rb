...
module Aggregation
  class ValidateAmrData
    ...
    def validate(debug_analysis: false)
      ...
      Rails.logger.debug { "Validating meter data of type #{@meter.meter_type} #{@meter.name} #{@meter.id}" }
      Rails.logger.debug { "Meter data from #{@meter.amr_data.start_date} to #{@meter.amr_data.end_date}" }
      Rails.logger.debug { "DCC Meter #{@meter.dcc_meter}" }
      ...
      if debug_analysis || Rails.logger.level >= Logger::INFO
    ...
    def auto_insert_for_gas_if_no_other_rules
      ...
      Rails.logger.info "Adding auto insert missing readings as we're a gas meter & no other corrections"
    ...
    def apply_one_meter_correction(rule)
      ...
      Rails.logger.debug { "Manually defined meter corrections: #{rule}" }
      if rule.is_a?(Symbol) && rule == :set_all_missing_to_zero
        Rails.logger.debug 'Setting all missing data to zero'
        ...
        Rails.logger.debug 'Correcting partially missing (zero) data'
        ...
        Rails.logger.debug 'Got missing summer rule'
    ...
    def apply_readings_start_date(fix_start_date)
      ...
      Rails.logger.debug { "Fixing start date to #{fix_start_date} for #{@meter_id}" }
    ...
    def apply_readings_end_date(fix_end_date)
      ...
      Rails.logger.debug { "Fixing end date to #{fix_end_date}" }
    ...
    def extend_start_date(date)
      Rails.logger.info "Extending start date to #{date}"
    ...
    def extend_end_date(date)
      Rails.logger.info "Extending end date to #{date}"
    ...
    def set_all_missing_data_to_zero_by_time_of_year(start_toy, end_toy, type)
      ...
      year_count.each do |year, count|
        Rails.logger.info "set during #{year} * #{count} to zero"
      end
    end

    def remove_dcc_bad_data_readings
      Rails.logger.info 'Checking dcc meter for bad values'
      too_much_bad_data = {}
      for_interpolation = {}
      ok_data = {}
      ...
      if too_much_bad_data.length.positive?
        Rails.logger.info 'The following dates have too much DCC bad data, so removing them for future whole day substitution'
      ...
      if for_interpolation.length.positive?
        Rails.logger.info 'The following dates have bad half hour kWh value, so nullifying for future interpolation'
      ...
      Rails.logger.info "Leaving #{ok_data.length} days of dcc data with no bad values"
    end
    ...
    def remove_final_meter_reading_if_today
      ...
      (Time.zone.today..@meter.amr_data.end_date).each do |date|
        ...
        Rails.logger.info "Removing final partial dcc meter reading for today #{date}"
      end
    end

    def log_dates(hash)
      hash.keys.each_slice(8) do |dates|
        Rails.logger.info dates.map { |d| d.strftime('%d-%b-%Y') }.join(' ')
      end
    end
    ...
    def remove_missing_start_end_dates_if_partial_nil
      if @amr_data.days_kwh_x48(@amr_data.start_date).any?(&:nil?)
        Rails.logger.info "Moving meter start date #{@amr_data.start_date} 1 day forward as only partial data on start date"
      ...
      Rails.logger.info "Moving meter end date #{@amr_data.end_date} one day back as only partial data on end date"
    ...
    def interpolate_partial_missing_data(max_missing_readings, missing_data_value)
      (@amr_data.start_date..@amr_data.end_date).each do |date|
        ...
        days_kwh_x48 = interpolate_zero_readings(days_kwh_x48, missing_data_value:)
    ...
    def substitute_partial_missing_data_with_whole_day(missing_dates)
      missing_dates.each do |date|
        ...
        if updated_one_day_reading.nil?
          Rails.logger.debug { "Unable to override partial/missing data for #{@meter_id} on #{date}" }
        else
    ...
    def set_missing_data_to_zero_on_heating_meter_during_summer(start_toy, end_toy)
      Rails.logger.info "Setting missing data to zero between #{start_toy} and #{end_toy}"
    ...
    def set_all_missing_data_to_zero
      Rails.logger.info 'Setting all missing data to zero'
    ...
    def scale_amr_data(start_date, end_date, scale)
      Rails.logger.info "Rescaling data between #{start_date} and #{end_date} by #{scale}"
      rescaler = Corrections::Rescaler.new(@amr_data, @meter_id)
      @amr_data = rescaler.perform(start_date:, end_date:, scale:)
    end
    ...
    def correct_holidays_with_adjacent_academic_years
      ...
      missing_holiday_dates.each do |date|
        ...
        if matching_holidays.empty?
          Rails.logger.info "Unable to find substitute matching holiday for date #{date}"
        else
          ...
          if adjusted_date.nil?
            ...
            Rails.logger.debug do
              "Correcting missing electricity holiday on #{date} with data from #{adjusted_date}"
            end
        ...
        Rails.logger.error "Comment: deliberately rescued missing holiday data exception for date #{date}"
    ...
    def similar_holiday(date, holiday_type, year_offset)
      ...
      Rails.logger.warn "Unable to find holiday of type #{holiday_type} and date #{date}  and offset #{year_offset}"
      nil
    end
    ...
    def replace_missing_data_with_zero(start_date, end_date)
      Rails.logger.info "Replacing missing data between #{start_date} and #{end_date} with zero"
    ...
    def zero_data_in_date_range(start_date, end_date)
      Rails.logger.info "Overwriting bad data between #{start_date} and #{end_date} with zero"
      (start_date..end_date).each do |date|
        next unless @amr_data.date_exists?(date)
    ...
    #
    # @param Date start the start date, or nil to use the meter start date
    # @param Date end the end date, or nil to use meter end date
    ...
    def zero_missing_data_in_date_range(start_date, end_date, zero_up_until_yesterday)
      ...
      Rails.logger.info "Setting missing data between #{start_date} and #{end_date} to zero"
      (start_date..end_date).each do |date|
        next unless @amr_data.date_missing?(date)
    ...
    def override_with_sheffield_solar_pv_data(start_date, end_date)
      ...
      Rails.logger.info "Correcting solar pv production data using Sheffield #{start_date} to #{end_date} current total kwh #{existing_kwh}"
      ...
      Rails.logger.info "Updated sheffield pv data kwh = #{updated_kwh}"
    end
    ...
    def fill_in_missing_data(start_date = @amr_data.start_date, end_date = @amr_data.end_date, sub_type_code = 'S', override = false)
    ...
    def average_temperature(date)
      ...
      begin
        ...
        Rails.logger.error "Warning: problem generating missing gas data, as no temperature data for #{date}"
        raise
      end
    end
    ...
    def adjusted_substitute_heating_kwh(missing_day, substitute_day)
      ...
      if prediction_ratio.negative?
        Rails.logger.debug do
          "Warning: negative predicated data for missing day #{missing_day} from #{substitute_day} setting to zero"
        end
        prediction_ratio = 0.0
      end

      if kwh_prediction_for_substitute_day == 0.0
        Rails.logger.warn "Warning: zero predicted kwh for substitute day #{substitute_day} setting prediction_ratio to 0.0 for #{missing_day}"
    ...
    def create_heating_model
      ...
      Rails.logger.info "Unable to calculate model data for heat day substitution for #{@meter_id}"
      Rails.logger.info e.message
      Rails.logger.info 'using simplistic substitution without modelling'
      NO_MODEL
    end
