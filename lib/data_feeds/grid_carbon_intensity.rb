...
module DataFeeds
  class GridCarbonIntensity < HalfHourlyData
    ...
    def parameterised_co2(date)
      hard_coded_mains_grid_intensity.select { |date_range|
        date >= date_range.first && date <= date_range.last
      }.values.first / 1_000.0
    end
    ...
    private_class_method def self.calculate_grid_carbon_intensity_for_year_kg(year)
      raise NotInYearRange, "not in year range #{year}" unless year.between?(
        grid_intensity_parameters.keys.first.first.year, grid_intensity_parameters.keys.last.last.year
      )
