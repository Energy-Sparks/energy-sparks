...
class FormatUnit
  ...
  ENERGY_UNITS      = 'analytics.energy_units'
  INFINITY          = 'analytics.units.infinity'
  NEGATIVE_INFINITY = 'analytics.units.infinity_negative'
  NAN               = 'analytics.units.nan'
  MINUTES           = 'analytics.units.minutes'
  HOURS             = 'analytics.units.hours'
  DAYS              = 'analytics.units.days'
  WEEKS             = 'analytics.units.weeks'
  MONTHS            = 'analytics.units.months'
  YEARS             = 'analytics.units.years'

  ZERO = '0'

  def self.percent_to_1_dp(val, medium = :html)
    I18n.t(key_for_unit(:percent, medium), value: Kernel.format('%.1f', val * 100.0))
  end
  ...
  private_class_method def self.process_unit(unit)
    unit_options = {}
    if unit.is_a?(Hash) && unit.key?(:options)
  ...
  private_class_method def self.format_private(unit, value, medium, convert_missing_types_to_strings, in_table,
                                               user_numeric_comprehension_level, unit_options)
    return value if medium == :raw || no_recent_or_not_enough_data?(value)
    #  && in_table - PH 20Nov2019 experimental change to tidying blank cells on heads summary table
    return '' if value.nil?
    ...
    # FormatUnit.format(row_units, val, :html, true, table_format, precision)
    ...
    elsif %i[timeofday fuel_type].include?(unit)
      value.to_s
    else
  ...
  private_class_method def self.format_percent(value, unit, user_numeric_comprehension_level, medium, unit_options)
    ...
    value *= 100.0 if unit_options.fetch(:scale, true)
  ...
  private_class_method def self.format_years_range(range)
    if range.first == range.last
      ...
      I18n.t(key_for_unit(:years_range), low: format_time(range.first), high: format_time(range.last))
    end
  end
  ...
  def self.format_pounds(unit, value, medium, user_numeric_comprehension_level, no_dp = false)
    ...
    if (value.respond_to?(:nan?) && value.nan?) || (value.respond_to?(:infinite?) && value.infinite?)
      ...
      # £-40.00 => -£40.00
      I18n.t(key_for_unit(unit, medium), sign: (value < 0.0 ? '-' : ''),
                                         value: scale_num(value.magnitude, true, user_numeric_comprehension_level))
    else
  ...
  def self.scale_num(value, in_pounds = false, user_numeric_comprehension_level = :default)
    ...
    after_decimal_point = if number_as_string.include?('.')
                            number_as_string.gsub!(/.*(\..*)/, '\1').gsub(/^.*\.0$/,
                                                                          '')
                          else
                            ''
                          end
  ...
  private_class_method def self.user_numeric_comprehension_level(user_type)
    ...
    when :default, :ks2, :benchmark, :target
      3
