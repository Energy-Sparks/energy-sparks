namespace :targets do
  ...
  task send_review_target: [:environment] do
    puts "#{Time.zone.now} send_review_target start - Sending set new target emails"
    ...
    puts "#{Time.zone.now} send_review_target end - Finished sending set new target emails"
  end
end
