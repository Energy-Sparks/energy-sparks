namespace :meters do
  desc 'Add heating non-heating attributes'
  task add_heating_non_heating_attributes: :environment do
    ...
    real_meters.each do |mpxn, attrs|
      puts "#{mpxn}, #{attrs.keys[0]}, #{attrs.values[0]}"

      type = attrs.keys[0]
      value = attrs.values[0]

      if meter = Meter.find_by_mpan_mprn(mpxn)
        puts meter.meter_attributes.inspect
        ...
        puts "No meter found for #{mpxn}"
      end
    end

    aggregated_gas.each do |urn, attrs|
      puts "#{urn}, #{attrs.keys[0]}, #{attrs.values[0]}"

      type = attrs.keys[0]
      value = attrs.values[0]
      meter_types = ["aggregated_gas"]
