namespace :after_party do
  ...
  task load_regions_and_local_authority_districts: :environment do
    ...
    file_name = File.join(__dir__, 'schools-to-lads-2023-07.csv')
    # id,RGN22NM,LAD22CD
    CSV.foreach(file_name, headers: true) do |row|
      school = School.find_by(id: row[0])
      if school.present?
        region = row[1].present? ? row[1].parameterize.underscore.to_sym : nil
        lad = LocalAuthorityArea.find_by(code: row[2])
        school.update!(region: region, local_authority_area: lad)
      else
        warn "Can't find school with id #{row[0]}"
      end
    end
